...
final class RicherScopeGetTypeHelper
{
    ...
    public function getIdenticalResult(\PHPStan\Analyser\Scope $scope, Identical $expr): TypeResult
    {
        ...
        if (($expr->left instanceof Node\Expr\PropertyFetch || $expr->left instanceof Node\Expr\StaticPropertyFetch) && $rightType->isNull()->yes() && !$scope->hasPropertyNativeType($expr->left)) {
            return new TypeResult(new BooleanType(), []);
        }
        if (($expr->right instanceof Node\Expr\PropertyFetch || $expr->right instanceof Node\Expr\StaticPropertyFetch) && $leftType->isNull()->yes() && !$scope->hasPropertyNativeType($expr->right)) {
            return new TypeResult(new BooleanType(), []);
        }
        return $this->initializerExprTypeResolver->resolveIdenticalType($leftType, $rightType);
    }

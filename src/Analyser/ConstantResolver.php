...
final class ConstantResolver
{
     ...
     */
    public function __construct(ReflectionProviderProvider $reflectionProviderProvider, array $dynamicConstantNames, $phpVersion, ComposerPhpVersionFactory $composerPhpVersionFactory)
    {
    ...
    public function resolvePredefinedConstant(string $resolvedConstantName): ?Type
    {
        ...
        $minPhpVersion = null;
        $maxPhpVersion = null;
        if (in_array($resolvedConstantName, ['PHP_VERSION_ID', 'PHP_MAJOR_VERSION', 'PHP_MINOR_VERSION', 'PHP_RELEASE_VERSION'], \true)) {
        ...
        if ($resolvedConstantName === 'PHP_MINOR_VERSION') {
            $minMinor = 0;
            $maxMinor = null;
            if ($minPhpVersion !== null && $maxPhpVersion !== null && $maxPhpVersion->getMajorVersionId() === $minPhpVersion->getMajorVersionId()) {
        ...
        if ($resolvedConstantName === 'PHP_RELEASE_VERSION') {
            $minRelease = 0;
            $maxRelease = null;
            if ($minPhpVersion !== null && $maxPhpVersion !== null && $maxPhpVersion->getMajorVersionId() === $minPhpVersion->getMajorVersionId() && $maxPhpVersion->getMinorVersionId() === $minPhpVersion->getMinorVersionId()) {

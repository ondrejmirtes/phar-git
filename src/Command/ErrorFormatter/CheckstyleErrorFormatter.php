...
use const ENT_XML1;
final class CheckstyleErrorFormatter implements \PHPStan\Command\ErrorFormatter\ErrorFormatter
{
    ...
    public function formatErrors(AnalysisResult $analysisResult, Output $output): int
    {
        ...
        foreach ($this->groupByFile($analysisResult) as $relativeFilePath => $errors) {
            ...
            foreach ($errors as $error) {
                $output->writeRaw(sprintf('  <error line="%d" column="1" severity="error" message="%s"%s />', $this->escape((string) $error->getLine()), $this->escape($error->getMessage()), $error->getIdentifier() !== null ? sprintf(' source="%s"', $this->escape($error->getIdentifier())) : ''));
                $output->writeLineFormatted('');
            }

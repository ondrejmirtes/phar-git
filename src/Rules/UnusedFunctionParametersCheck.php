...
final class UnusedFunctionParametersCheck
{
    ...
    public function __construct(ReflectionProvider $reflectionProvider, bool $reportExactLine)
    {
        $this->reflectionProvider = $reflectionProvider;
        $this->reportExactLine = $reportExactLine;
    }
    /**
    ...
    public function getUnusedParameters(Scope $scope, array $parameterVars, array $statements, string $unusedParameterMessage, string $identifier): array
    {
        ...
        foreach ($unusedParameters as $name => $variable) {
            $errorBuilder = \PHPStan\Rules\RuleErrorBuilder::message(sprintf($unusedParameterMessage, $name))->identifier($identifier);

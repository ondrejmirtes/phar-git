...
{
    private \PHPStan\Rules\Comparison\ConstantConditionRuleHelper $constantConditionRuleHelper;
    private bool $reportAlwaysTrueInLastCondition;
    private bool $treatPhpDocTypesAsCertain;
    public function __construct(\PHPStan\Rules\Comparison\ConstantConditionRuleHelper $constantConditionRuleHelper, bool $reportAlwaysTrueInLastCondition, bool $treatPhpDocTypesAsCertain)
    {
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        foreach ($node->getArms() as $i => $arm) {
            ...
            foreach ($armConditions as $armCondition) {
                ...
                if (!$armConditionResult->getValue()) {
                    $errors[] = RuleErrorBuilder::message(sprintf('Match arm comparison between %s and %s is always false.', $armConditionScope->getType($matchCondition)->describe(VerbosityLevel::value()), $armConditionScope->getType($armCondition->getCondition())->describe(VerbosityLevel::value())))->line($armLine)->identifier('match.alwaysFalse')->build();
                    continue;
                }
        ...
        if (!$hasDefault && !$nextArmIsDeadForType) {
            ...
            if (!$remainingType instanceof NeverType && !$this->isUnhandledMatchErrorCaught($node) && !$this->hasUnhandledMatchErrorThrowsTag($scope)) {
                $errors[] = RuleErrorBuilder::message(sprintf('Match expression does not handle remaining %s: %s', $remainingType instanceof UnionType ? 'values' : 'value', $remainingType->describe(VerbosityLevel::value())))->identifier('match.unhandled')->build();
            }
        }
        return $errors;
    }

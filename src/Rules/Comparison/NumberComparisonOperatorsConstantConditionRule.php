...
final class NumberComparisonOperatorsConstantConditionRule implements Rule
{
    ...
    public function __construct(bool $treatPhpDocTypesAsCertain, bool $treatPhpDocTypesAsCertainTip)
    {
    ...
    public function processNode(Node $node, Scope $scope) : array
    {
        ...
        if ($exprType instanceof ConstantBooleanType) {
            ...
            return [$addTip(RuleErrorBuilder::message(sprintf('Comparison operation "%s" between %s and %s is always %s.', $node->getOperatorSigil(), $scope->getType($node->left)->describe(VerbosityLevel::value()), $scope->getType($node->right)->describe(VerbosityLevel::value()), $exprType->getValue() ? 'true' : 'false')))->identifier(sprintf('%s.always%s', $nodeType, $exprType->getValue() ? 'True' : 'False'))->build()];
        }
        return [];
    }
}

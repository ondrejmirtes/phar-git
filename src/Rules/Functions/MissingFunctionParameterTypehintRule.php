...
final class MissingFunctionParameterTypehintRule implements Rule
{
    ...
    private function checkFunctionParameter(FunctionReflection $functionReflection, string $parameterMessage, Type $parameterType) : array
    {
        ...
        foreach ($this->missingTypehintCheck->getIterableTypesWithMissingValueTypehint($parameterType) as $iterableType) {
            ...
            $messages[] = RuleErrorBuilder::message(sprintf('Function %s() has %s with no value type specified in iterable type %s.', $functionReflection->getName(), $parameterMessage, $iterableTypeDescription))->tip(MissingTypehintCheck::MISSING_ITERABLE_VALUE_TYPE_TIP)->identifier('missingType.iterableValue')->build();
        }
        foreach ($this->missingTypehintCheck->getNonGenericObjectTypesWithGenericClass($parameterType) as [$name, $genericTypeNames]) {
            $messages[] = RuleErrorBuilder::message(sprintf('Function %s() has %s with generic %s but does not specify its types: %s', $functionReflection->getName(), $parameterMessage, $name, $genericTypeNames))->identifier('missingType.generics')->build();
        }
        foreach ($this->missingTypehintCheck->getCallablesWithMissingSignature($parameterType) as $callableType) {
            $messages[] = RuleErrorBuilder::message(sprintf('Function %s() has %s with no signature specified for %s.', $functionReflection->getName(), $parameterMessage, $callableType->describe(VerbosityLevel::typeOnly())))->identifier('missingType.callable')->build();
        }
        return $messages;
    }
}

...
final class PrintfArrayParametersRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        if (!in_array($name, ['vprintf', 'vsprintf'], \true)) {
            return [];
        }
        $args = $node->getArgs();
        ...
        if (!$this->placeholdersMatchesArgsCount($placeHoldersCount, $formatArgsCount)) {
            ...
            return [RuleErrorBuilder::message(sprintf(sprintf('%s, %s.', $singlePlaceholder ? 'Call to %s contains %d placeholder' : 'Call to %s contains %s placeholders', $singleValue ? '%d value given' : '%s values given'), $name, $placeholders, $values))->identifier(sprintf('argument.%s', $name))->build()];
        }
        return [];
    }
    /**
    ...
    private function placeholdersMatchesArgsCount($placeHoldersCount, $formatArgsCount): bool
    {
        ...
        if ($formatArgsCount instanceof IntegerRangeType && IntegerRangeType::fromInterval(1, null)->isSuperTypeOf($placeHoldersCount)->yes()) {

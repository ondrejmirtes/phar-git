...
final class CallCallablesRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope) : array
    {
        ...
        $typeResult = $this->ruleLevelHelper->findTypeToCheck($scope, NullsafeOperatorHelper::getNullsafeShortcircuitedExprRespectingScope($scope, $node->name), 'Invoking callable on an unknown class %s.', static fn(Type $type): bool => $type->isCallable()->yes());
        ...
        if ($isCallable->no()) {
            return [RuleErrorBuilder::message(sprintf('Trying to invoke %s but it\'s not a callable.', $type->describe(VerbosityLevel::value())))->identifier('callable.nonCallable')->build()];
        }
        if ($this->reportMaybes && $isCallable->maybe()) {
            return [RuleErrorBuilder::message(sprintf('Trying to invoke %s but it might not be a callable.', $type->describe(VerbosityLevel::value())))->identifier('callable.nonCallable')->build()];
        }
        ...
        if (count($parametersAcceptors) === 1 && $parametersAcceptors[0] instanceof InaccessibleMethod) {
            ...
            $messages[] = RuleErrorBuilder::message(sprintf('Call to %s method %s() of class %s.', $method->isPrivate() ? 'private' : 'protected', $method->getName(), $method->getDeclaringClass()->getDisplayName()))->identifier('callable.inaccessibleMethod')->build();
        }
        $parametersAcceptor = ParametersAcceptorSelector::selectFromArgs($scope, $node->getArgs(), $parametersAcceptors, null);

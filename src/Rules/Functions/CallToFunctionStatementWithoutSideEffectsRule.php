...
final class CallToFunctionStatementWithoutSideEffectsRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        if (in_array($functionName, self::PHPSTAN_TESTING_FUNCTIONS, \true)) {
            return [];
        }
        if (isset(self::SIDE_EFFECT_FLIP_PARAMETERS[$functionName])) {
            [$flipParameterName, $flipParameterPosition, $testName] = self::SIDE_EFFECT_FLIP_PARAMETERS[$functionName];
            $sideEffectFlipped = \false;
            $hasNamedParameter = \false;
            $checker = ['isNotNull' => static fn(Type $type) => $type->isNull()->no(), 'isTruthy' => static fn(Type $type) => $type->toBoolean()->isTrue()->yes()][$testName];
        ...
        if (!$functionHasSideEffects || $node->expr->isFirstClassCallable()) {
            ...
            return [RuleErrorBuilder::message(sprintf('Call to function %s() on a separate line has no effect.', $function->getName()))->identifier('function.resultUnused')->build()];
        }
        return [];
    }
}

...
final class ArrayValuesRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope) : array
    {
        ...
        if ($arrayType->isIterableAtLeastOnce()->no()) {
            ...
            $errorBuilder = RuleErrorBuilder::message(sprintf($message, $arrayType->describe(VerbosityLevel::value())))->identifier('arrayValues.empty');
            if ($this->treatPhpDocTypesAsCertain) {
        ...
        if ($arrayType->isList()->yes()) {
            ...
            $errorBuilder = RuleErrorBuilder::message(sprintf($message, $arrayType->describe(VerbosityLevel::value())))->identifier('arrayValues.list');
            if ($this->treatPhpDocTypesAsCertain) {
                $nativeArrayType = $scope->getNativeType($args[0]->value);

...
final class RandomIntParametersRule implements Rule
{
    ...
    public function __construct(ReflectionProvider $reflectionProvider, PhpVersion $phpVersion, bool $reportMaybes)
    {
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        if ($isSmaller->yes() || $isSmaller->maybe() && $this->reportMaybes) {
            ...
            return [RuleErrorBuilder::message(sprintf($message, $minType->describe(VerbosityLevel::value()), $maxType->describe(VerbosityLevel::value())))->identifier('argument.type')->build()];
        }
        return [];
    }
}

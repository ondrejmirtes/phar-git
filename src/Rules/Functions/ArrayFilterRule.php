...
final class ArrayFilterRule implements Rule
{
    ...
    public function __construct(ReflectionProvider $reflectionProvider, bool $treatPhpDocTypesAsCertain, bool $treatPhpDocTypesAsCertainTip)
    {
    ...
    public function processNode(Node $node, Scope $scope) : array
    {
        ...
        if ($arrayType->isIterableAtLeastOnce()->no()) {
            ...
            $errorBuilder = RuleErrorBuilder::message(sprintf($message, $arrayType->describe(VerbosityLevel::value())))->identifier('arrayFilter.empty');
            if ($this->treatPhpDocTypesAsCertain) {
        ...
        if ($isSuperType->no()) {
            ...
            $errorBuilder = RuleErrorBuilder::message(sprintf($message, $arrayType->describe(VerbosityLevel::value())))->identifier('arrayFilter.same');
            if ($this->treatPhpDocTypesAsCertain) {
                $nativeArrayType = $scope->getNativeType($args[0]->value);
        ...
        if ($isSuperType->yes()) {
            ...
            $errorBuilder = RuleErrorBuilder::message(sprintf($message, $arrayType->describe(VerbosityLevel::value())))->identifier('arrayFilter.alwaysEmpty');
            if ($this->treatPhpDocTypesAsCertain) {
                $nativeArrayType = $scope->getNativeType($args[0]->value);
                $isNativeSuperType = $falsyType->isSuperTypeOf($nativeArrayType->getIterableValueType());

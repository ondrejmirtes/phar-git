...
final class InvalidAssignVarRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope) : array
    {
        ...
        if ($this->nullsafeCheck->containsNullSafe($node->var)) {
            return [RuleErrorBuilder::message('Nullsafe operator cannot be on left side of assignment.')->identifier('nullsafe.assign')->nonIgnorable()->build()];
        }
        if ($node instanceof AssignRef && $this->nullsafeCheck->containsNullSafe($node->expr)) {
            return [RuleErrorBuilder::message('Nullsafe operator cannot be on right side of assignment by reference.')->identifier('nullsafe.byRef')->nonIgnorable()->build()];
        }
        if ($this->containsNonAssignableExpression($node->var)) {
            return [RuleErrorBuilder::message('Expression on left side of assignment is not assignable.')->identifier('assign.invalidExpr')->nonIgnorable()->build()];
        }
        return [];
    }

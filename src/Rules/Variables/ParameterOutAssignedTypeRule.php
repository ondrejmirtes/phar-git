...
final class ParameterOutAssignedTypeRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope) : array
    {
        ...
        $typeResult = $this->ruleLevelHelper->findTypeToCheck($scope, $node->getAssignedExpr(), '', static fn(Type $type): bool => $outType->isSuperTypeOf($type)->yes());
        ...
        $errorBuilder = RuleErrorBuilder::message(sprintf('Parameter &$%s %s of %s expects %s, %s given.', $foundParameter->getName(), $isParamOutType ? '@param-out type' : 'by-ref type', $functionDescription, $outType->describe($verbosityLevel), $assignedExprType->describe($verbosityLevel)))->identifier(sprintf('%s.type', $isParamOutType ? 'paramOut' : 'parameterByRef'));
        if (!$isParamOutType) {

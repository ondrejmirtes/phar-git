...
final class VarianceCheck
{
    ...
    public function checkParametersAcceptor(ExtendedParametersAcceptor $parametersAcceptor, string $parameterTypeMessage, string $parameterOutTypeMessage, string $returnTypeMessage, string $generalMessage, bool $isStatic, bool $isPrivate, string $identifier): array
    {
        ...
        foreach ($parametersAcceptor->getTemplateTypeMap()->getTypes() as $templateType) {
            ...
            $errors[] = RuleErrorBuilder::message(sprintf('Variance annotation is only allowed for type parameters of classes and interfaces, but occurs in template type %s in %s.', $templateType->getName(), $generalMessage))->identifier(sprintf('%s.variance', $identifier))->build();
        }
        if ($isPrivate) {
            return $errors;
        }
    ...
    public function check(TemplateTypeVariance $positionVariance, Type $type, string $messageContext): array
    {
        ...
        foreach ($type->getReferencedTemplateTypes($positionVariance) as $reference) {
            ...
            $errors[] = RuleErrorBuilder::message(sprintf('Template type %s is declared as %s, but occurs in %s position %s.', $referredType->getName(), $referredType->getVariance()->describe(), $reference->getPositionVariance()->describe(), $messageContext))->identifier('generics.variance')->build();
        }
        return $errors;
    }

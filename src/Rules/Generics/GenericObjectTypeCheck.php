    ...
    public function check(Type $phpDocType, string $classNotGenericMessage, string $notEnoughTypesMessage, string $extraTypesMessage, string $typeIsNotSubtypeMessage, string $typeProjectionHasConflictingVarianceMessage, string $typeProjectionIsRedundantMessage): array
    {
        ...
        foreach ($genericTypes as $genericType) {
            ...
            if (!$classReflection->isGeneric()) {
                $messages[] = RuleErrorBuilder::message(sprintf($classNotGenericMessage, $genericType->describe(VerbosityLevel::typeOnly()), $classLikeDescription, $classReflection->getDisplayName()))->identifier('generics.notGeneric')->build();
                continue;
            }
            ...
            if ($requiredTemplateTypesCount > $genericTypeTypesCount) {
                ...
                $messages[] = RuleErrorBuilder::message(sprintf($notEnoughTypesMessage, $genericType->describe(VerbosityLevel::typeOnly()), $classLikeDescription, $classReflection->getDisplayName(\false), $templateTypesList))->identifier('generics.lessTypes')->build();
            } elseif ($templateTypesCount < $genericTypeTypesCount) {
                $templateTypesList = implode(', ', array_keys($classReflection->getTemplateTypeMap()->getTypes()));
                if ($requiredTemplateTypesCount !== $templateTypesCount) {
                    $templateTypesList .= sprintf(' (%d-%d required)', $requiredTemplateTypesCount, $templateTypesCount);
                }
                $messages[] = RuleErrorBuilder::message(sprintf($extraTypesMessage, $genericType->describe(VerbosityLevel::typeOnly()), $genericTypeTypesCount, $classLikeDescription, $classReflection->getDisplayName(\false), $templateTypesCount, $templateTypesList))->identifier('generics.moreTypes')->build();
            }
            for ($i = 0; $i < $templateTypesCount; $i++) {
                ...
                $messages[] = RuleErrorBuilder::message(sprintf($typeIsNotSubtypeMessage, $genericTypeType->describe(VerbosityLevel::typeOnly()), $genericType->describe(VerbosityLevel::typeOnly()), $templateType->describe(VerbosityLevel::typeOnly()), $classLikeDescription, $classReflection->getDisplayName(\false)))->identifier('generics.notSubtype')->build();
            }
        }
        return $messages;
    }

    ...
    public function processNode(Node $node, Scope $scope) : array
    {
        ...
        foreach ($parentStmtTypes as $parentStmtType) {
            ...
            if ($parentStmtType === Node\Expr\Closure::class) {
                return [RuleErrorBuilder::message(sprintf('Keyword %s used outside of a loop or a switch statement.', $node instanceof Stmt\Continue_ ? 'continue' : 'break'))->nonIgnorable()->identifier(sprintf('%s.outOfLoop', $node instanceof Stmt\Continue_ ? 'continue' : 'break'))->build()];
            }

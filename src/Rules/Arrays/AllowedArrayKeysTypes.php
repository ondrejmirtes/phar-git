...
final class AllowedArrayKeysTypes
{
    ...
    public static function narrowOffsetKeyType(Type $varType, Type $keyType): ?Type
    {
        ...
        if ($varIterableKeyType->isConstantScalarValue()->yes()) {
            $narrowedKey = TypeCombinator::union($varIterableKeyType, TypeCombinator::remove($varIterableKeyType->toString(), new ConstantStringType('')));
            if (!$varType->hasOffsetValueType(new ConstantIntegerType(0))->no()) {
                $narrowedKey = TypeCombinator::union($narrowedKey, new ConstantBooleanType(\false));
            }
            if (!$varType->hasOffsetValueType(new ConstantIntegerType(1))->no()) {
                $narrowedKey = TypeCombinator::union($narrowedKey, new ConstantBooleanType(\true));
            }
        ...
        return new MixedType(\false, new UnionType([new ArrayType(new MixedType(), new MixedType()), new ObjectWithoutClassType(), new ResourceType()]));
    }
}

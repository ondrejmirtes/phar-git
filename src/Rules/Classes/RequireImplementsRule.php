    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        foreach ($classReflection->getTraits(\true) as $trait) {
            ...
            foreach ($implementsTags as $implementsTag) {
                ...
                $errors[] = RuleErrorBuilder::message(sprintf('Trait %s requires using class to implement %s, but %s does not.', $trait->getDisplayName(), $type->describe(VerbosityLevel::typeOnly()), $classReflection->getDisplayName()))->identifier('class.missingImplements')->build();
            }
        }
        return $errors;
    }

...
final class ImpossibleInstanceOfRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope) : array
    {
        ...
        if (!$instanceofType->getValue()) {
            ...
            return [$addTip(RuleErrorBuilder::message(sprintf('Instanceof between %s and %s will always evaluate to false.', $exprType->describe(VerbosityLevel::typeOnly()), $classType->describe(VerbosityLevel::getRecommendedLevelByType($classType)))))->identifier('instanceof.alwaysFalse')->build()];
        }

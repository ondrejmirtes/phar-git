...
final class LocalTypeAliasesCheck
{
     ...
     */
    public function __construct(array $globalTypeAliases, ReflectionProvider $reflectionProvider, TypeNodeResolver $typeNodeResolver, MissingTypehintCheck $missingTypehintCheck, ClassNameCheck $classCheck, UnresolvableTypeHelper $unresolvableTypeHelper, GenericObjectTypeCheck $genericObjectTypeCheck, bool $checkMissingTypehints, bool $checkClassCaseSensitivity, bool $discoveringSymbolsTip)
    {
    ...
    public function checkInTraitDefinitionContext(ClassReflection $reflection): array
    {
        ...
        foreach ($phpDoc->getTypeAliasImportTags() as $typeAliasImportTag) {
            ...
            if (!$this->reflectionProvider->hasClass($importedFromClassName)) {
                $errors[] = RuleErrorBuilder::message(sprintf('Cannot import type alias %s: class %s does not exist.', $importedAlias, $importedFromClassName))->identifier('class.notFound')->build();
                continue;
            }
            ...
            if (!array_key_exists($importedAlias, $typeAliases)) {
                $errors[] = RuleErrorBuilder::message(sprintf('Cannot import type alias %s: type alias does not exist in %s.', $importedAlias, $importedFromClassName))->identifier('typeAlias.notFound')->build();
                continue;
            }
        ...
        foreach ($phpDoc->getTypeAliasTags() as $typeAliasTag) {
            ...
            if (!$this->isAliasNameValid($aliasName, $nameScope)) {
                $errors[] = RuleErrorBuilder::message(sprintf('Type alias has an invalid name: %s.', $aliasName))->identifier('typeAlias.invalidName')->build();
                continue;
            }
            ...
            foreach ($this->missingTypehintCheck->getIterableTypesWithMissingValueTypehint($resolvedType) as $iterableType) {
                ...
                $errors[] = RuleErrorBuilder::message(sprintf('%s %s has type alias %s with no value type specified in iterable type %s.', $reflection->getClassTypeDescription(), $reflection->getDisplayName(), $aliasName, $iterableTypeDescription))->tip(MissingTypehintCheck::MISSING_ITERABLE_VALUE_TYPE_TIP)->identifier('missingType.iterableValue')->build();
            }
            foreach ($this->missingTypehintCheck->getNonGenericObjectTypesWithGenericClass($resolvedType) as [$name, $genericTypeNames]) {
                $errors[] = RuleErrorBuilder::message(sprintf('%s %s has type alias %s with generic %s but does not specify its types: %s', $reflection->getClassTypeDescription(), $reflection->getDisplayName(), $aliasName, $name, $genericTypeNames))->identifier('missingType.generics')->build();
            }
            foreach ($this->missingTypehintCheck->getCallablesWithMissingSignature($resolvedType) as $callableType) {
                $errors[] = RuleErrorBuilder::message(sprintf('%s %s has type alias %s with no signature specified for %s.', $reflection->getClassTypeDescription(), $reflection->getDisplayName(), $aliasName, $callableType->describe(VerbosityLevel::typeOnly())))->identifier('missingType.callable')->build();
            }
        }
        return $errors;
    }
    ...
    private function hasErrorType(Type $type, string $aliasName, array &$errors): bool
    {
        ...
        TypeTraverser::map($type, static function (Type $type, callable $traverse) use (&$errors, &$foundError, $aliasName): Type {
            ...
            if ($type instanceof CircularTypeAliasErrorType) {
                $errors[] = RuleErrorBuilder::message(sprintf('Circular definition detected in type alias %s.', $aliasName))->identifier('typeAlias.circular')->build();
                $foundError = \true;
                return $type;
            }
            if ($type instanceof ErrorType) {
                $errors[] = RuleErrorBuilder::message(sprintf('Invalid type definition detected in type alias %s.', $aliasName))->identifier('typeAlias.invalidType')->build();
                $foundError = \true;
                return $type;
            }

...
final class InvalidPHPStanDocTagRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope) : array
    {
        ...
        foreach ($phpDocNode->getTags() as $phpDocTag) {
            if (!str_starts_with($phpDocTag->name, '@phpstan-') || in_array($phpDocTag->name, self::POSSIBLE_PHPSTAN_TAGS, \true)) {
                continue;
            }
            $errors[] = RuleErrorBuilder::message(sprintf('Unknown PHPDoc tag: %s', $phpDocTag->name))->line(\PHPStan\Rules\PhpDoc\PhpDocLineHelper::detectLine($node, $phpDocTag))->identifier('phpDoc.phpstanTag')->build();
        }
        return $errors;
    }
}

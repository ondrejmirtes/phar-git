...
final class IncompatibleParamImmediatelyInvokedCallableRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope) : array
    {
        ...
        foreach ($node->getParams() as $parameter) {
            ...
            $nativeParameterTypes[$parameter->var->name] = $scope->getFunctionType($parameter->type, $scope->isParameterValueNullable($parameter), \false);
        }
        $errors = [];
        foreach ($resolvedPhpDoc->getParamsImmediatelyInvokedCallable() as $parameterName => $immediately) {
            ...
            if (!isset($nativeParameterTypes[$parameterName])) {
                ...
                $errors[] = RuleErrorBuilder::message(sprintf('PHPDoc tag %s is for parameter $%s with non-callable type %s.', $tagName, $parameterName, $nativeParameterTypes[$parameterName]->describe(VerbosityLevel::typeOnly())))->identifier(sprintf('%s.nonCallable', $immediately ? 'paramImmediatelyInvokedCallable' : 'paramLaterInvokedCallable'))->build();
            }
        }
        return $errors;
    }

...
final class GenericCallableRuleHelper
{
    ...
    public function __construct(TemplateTypeCheck $templateTypeCheck)
    {
        ...
        TypeTraverser::map($callableType, function (Type $type, callable $traverse) use (&$errors, $node, $scope, $location, $functionName, $functionTemplateTags, $classReflection) {
            ...
            if ($functionName !== null) {
                ...
                foreach (array_keys($functionTemplateTags) as $name) {
                    ...
                    $errors[] = RuleErrorBuilder::message(sprintf('PHPDoc tag %s template %s of %s shadows @template %s for %s.', $location, $name, $typeDescription, $name, $functionDescription))->identifier('callable.shadowTemplate')->build();
                }
            }
            if ($classReflection !== null) {
                foreach (array_keys($classReflection->getTemplateTags()) as $name) {
                    ...
                    $errors[] = RuleErrorBuilder::message(sprintf('PHPDoc tag %s template %s of %s shadows @template %s for class %s.', $location, $name, $typeDescription, $name, $classDescription))->identifier('callable.shadowTemplate')->build();
                }
            }
            return $traverse($type);
        });

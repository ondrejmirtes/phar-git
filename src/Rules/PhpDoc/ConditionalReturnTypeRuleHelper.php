        ...
        foreach ($conditionalTypes as $conditionalType) {
            if ($conditionalType instanceof ConditionalType) {
                ...
                if (count($templateTypes) === 0) {
                    $errors[] = RuleErrorBuilder::message(sprintf('Conditional return type uses subject type %s which is not part of PHPDoc @template tags.', $subjectType->describe(VerbosityLevel::typeOnly())))->identifier('conditionalType.subjectNotFound')->build();
                    continue;
                }
            } else {
                $parameterName = substr($conditionalType->getParameterName(), 1);
                if (!array_key_exists($parameterName, $parametersByName)) {
                    $errors[] = RuleErrorBuilder::message(sprintf('Conditional return type references unknown parameter $%s.', $parameterName))->identifier('parameter.notFound')->build();
                    continue;
                }

...
final class WrongVariableNameInVarTagRule implements Rule
{
    ...
    private \PHPStan\Rules\PhpDoc\VarTagTypeRuleHelper $varTagTypeRuleHelper;
    public function __construct(FileTypeMapper $fileTypeMapper, \PHPStan\Rules\PhpDoc\VarTagTypeRuleHelper $varTagTypeRuleHelper)
    {
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        if ($node instanceof InClassNode || $node instanceof InClassMethodNode || $node instanceof InFunctionNode) {
            ...
            return [RuleErrorBuilder::message(sprintf('PHPDoc tag @var above %s has no effect.', $description))->identifier('varTag.misplaced')->build()];
        }
    ...
    private function processAssign(Scope $scope, Node\Expr $var, Node\Expr $expr, array $varTags): array
    {
        ...
        foreach (array_keys($varTags) as $key) {
            if (is_int($key)) {
                if (count($varTags) !== 1) {
                    if (!$hasMultipleMessage) {
                        $errors[] = RuleErrorBuilder::message('Multiple PHPDoc @var tags above single variable assignment are not supported.')->identifier('varTag.multipleTags')->build();
                        $hasMultipleMessage = \true;
                    }
                } elseif (count($assignedVariables) !== 1) {
                    $errors[] = RuleErrorBuilder::message('PHPDoc tag @var above assignment does not specify variable name.')->identifier('varTag.noVariable')->build();
                }
                continue;
            }
            ...
            if (count($assignedVariables) === 1 && count($varTags) === 1) {
                $errors[] = RuleErrorBuilder::message(sprintf('Variable $%s in PHPDoc tag @var does not match assigned variable $%s.', $key, $assignedVariables[0]))->identifier('varTag.differentVariable')->build();
            } else {
                $errors[] = RuleErrorBuilder::message(sprintf('Variable $%s in PHPDoc tag @var does not exist.', $key))->identifier('varTag.variableNotFound')->build();
            }
        }
        if (count($errors) === 0) {
    ...
    private function processForeach(Scope $scope, Node\Expr $iterateeExpr, ?Node\Expr $keyVar, Node\Expr $valueVar, array $varTags): array
    {
        ...
        foreach (array_keys($varTags) as $name) {
            if (is_int($name)) {
                ...
                $errors[] = RuleErrorBuilder::message('PHPDoc tag @var above foreach loop does not specify variable name.')->identifier('varTag.noVariable')->build();
                continue;
            }
            if (in_array($name, $variableNames, \true)) {
                continue;
            }
            $errors[] = RuleErrorBuilder::message(sprintf('Variable $%s in PHPDoc tag @var does not match any variable in the foreach loop: %s', $name, implode(', ', array_map(static fn(string $name): string => sprintf('$%s', $name), $variableNames))))->identifier('varTag.differentVariable')->build();
        }
    ...
    private function processStatic(Scope $scope, array $vars, array $varTags): array
    {
        ...
        foreach (array_keys($varTags) as $name) {
            if (is_int($name)) {
                ...
                $errors[] = RuleErrorBuilder::message('PHPDoc tag @var above multiple static variables does not specify variable name.')->identifier('varTag.noVariable')->build();
                continue;
            }
            if (in_array($name, $variableNames, \true)) {
                continue;
            }
            $errors[] = RuleErrorBuilder::message(sprintf('Variable $%s in PHPDoc tag @var does not match any static variable: %s', $name, implode(', ', array_map(static fn(string $name): string => sprintf('$%s', $name), $variableNames))))->identifier('varTag.differentVariable')->build();
        }
        foreach ($vars as $var) {
            if ($var->default === null) {
                continue;
    ...
    private function processStmt(Scope $scope, array $varTags, ?Expr $defaultExpr): array
    {
        ...
        foreach ($varTags as $name => $varTag) {
            ...
            $errors[] = RuleErrorBuilder::message(sprintf('Variable $%s in PHPDoc tag @var does not exist.', $name))->identifier('varTag.variableNotFound')->build();
        }
        if (count($variableLessVarTags) !== 1 || $defaultExpr === null) {
            if (count($variableLessVarTags) > 0) {
                $errors[] = RuleErrorBuilder::message('PHPDoc tag @var does not specify variable name.')->identifier('varTag.noVariable')->build();
            }
        }
        return $errors;
    }
    ...
    private function processGlobal(Scope $scope, Node\Stmt\Global_ $node, array $varTags): array
    {
        ...
        foreach (array_keys($varTags) as $name) {
            if (is_int($name)) {
                ...
                $errors[] = RuleErrorBuilder::message('PHPDoc tag @var above multiple global variables does not specify variable name.')->identifier('varTag.noVariable')->build();
                continue;
            }
            if (isset($variableNames[$name])) {
                continue;
            }
            $errors[] = RuleErrorBuilder::message(sprintf('Variable $%s in PHPDoc tag @var does not match any global variable: %s', $name, implode(', ', array_map(static fn(string $name): string => sprintf('$%s', $name), array_keys($variableNames)))))->identifier('varTag.differentVariable')->build();
        }
        return $errors;
    }
}

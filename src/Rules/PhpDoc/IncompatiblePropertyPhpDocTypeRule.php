...
final class IncompatiblePropertyPhpDocTypeRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        if ($this->unresolvableTypeHelper->containsUnresolvableType($phpDocType)) {
            $messages[] = RuleErrorBuilder::message(sprintf('%s for property %s::$%s contains unresolvable type.', $description, $classReflection->getDisplayName(), $propertyName))->identifier('property.unresolvableType')->build();
        }
        $nativeType = $node->getNativeType();
        if ($nativeType !== null) {
            $isSuperType = $nativeType->isSuperTypeOf($phpDocType);
            if ($isSuperType->no()) {
                $messages[] = RuleErrorBuilder::message(sprintf('%s for property %s::$%s with type %s is incompatible with native type %s.', $description, $classReflection->getDisplayName(), $propertyName, $phpDocType->describe(VerbosityLevel::typeOnly()), $nativeType->describe(VerbosityLevel::typeOnly())))->identifier('property.phpDocType')->build();
            } elseif ($isSuperType->maybe()) {
                $errorBuilder = RuleErrorBuilder::message(sprintf('%s for property %s::$%s with type %s is not subtype of native type %s.', $description, $classReflection->getDisplayName(), $propertyName, $phpDocType->describe(VerbosityLevel::typeOnly()), $nativeType->describe(VerbosityLevel::typeOnly())))->identifier('property.phpDocType');
        ...
        if ($node->isPromoted() === \false) {
            $messages = array_merge($messages, $this->genericCallableRuleHelper->check($node, $scope, '@var', $phpDocType, null, [], $classReflection));
        }

...
final class RegularExpressionQuotingRule implements Rule
{
    ...
    public function __construct(ReflectionProvider $reflectionProvider, RegexExpressionHelper $regexExpressionHelper)
    {
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        if (!in_array($functionReflection->getName(), ['preg_match', 'preg_match_all', 'preg_filter', 'preg_grep', 'preg_replace', 'preg_replace_callback', 'preg_split'], \true)) {
            return [];
        }
    ...
    private function validateQuoteDelimiters(Concat $concat, Scope $scope, array $patternDelimiters): array
    {
        ...
        $errors = [];
        if ($concat->left instanceof FuncCall && $concat->left->name instanceof Name && $concat->left->name->toLowerString() === 'preg_quote') {
        ...
        if ($concat->right instanceof FuncCall && $concat->right->name instanceof Name && $concat->right->name->toLowerString() === 'preg_quote') {
    ...
    private function validatePregQuote(FuncCall $pregQuote, Scope $scope, array $patternDelimiters): ?IdentifierRuleError
    {
        ...
        if (count($args) === 1) {
            if (count($patternDelimiters) === 1) {
                return RuleErrorBuilder::message(sprintf('Call to preg_quote() is missing delimiter %s to be effective.', $patternDelimiters[0]))->line($pregQuote->getStartLine())->identifier('argument.invalidPregQuote')->build();
            }
            return RuleErrorBuilder::message('Call to preg_quote() is missing delimiter parameter to be effective.')->line($pregQuote->getStartLine())->identifier('argument.invalidPregQuote')->build();
        }
        if (count($args) >= 2) {
            foreach ($scope->getType($args[1]->value)->getConstantStrings() as $quoteDelimiterType) {
                ...
                if (!in_array($quoteDelimiter, $patternDelimiters, \true)) {
                    if (count($patternDelimiters) === 1) {
                        return RuleErrorBuilder::message(sprintf('Call to preg_quote() uses invalid delimiter %s while pattern uses %s.', $quoteDelimiter, $patternDelimiters[0]))->line($pregQuote->getStartLine())->identifier('argument.invalidPregQuote')->build();
                    }
                    return RuleErrorBuilder::message(sprintf('Call to preg_quote() uses invalid delimiter %s.', $quoteDelimiter))->line($pregQuote->getStartLine())->identifier('argument.invalidPregQuote')->build();
                }
            }
        }
        return null;

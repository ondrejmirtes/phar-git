...
final class FileAssertRule implements Rule
{
    ...
    private function processAssertType(array $args, Scope $scope): array
    {
        ...
        if (count($expectedTypeStrings) !== 1) {
            return [RuleErrorBuilder::message('Expected type must be a literal string.')->nonIgnorable()->identifier('phpstan.unknownExpectation')->build()];
        }
        ...
        return [RuleErrorBuilder::message(sprintf('Expected type %s, actual: %s', $expectedTypeStrings[0]->getValue(), $expressionType))->nonIgnorable()->identifier('phpstan.type')->build()];
    }
    /**
     * @param Node\Arg[] $args
     * @return list<IdentifierRuleError>
    ...
    private function processAssertNativeType(array $args, Scope $scope): array
    {
        ...
        if (count($expectedTypeStrings) !== 1) {
            return [RuleErrorBuilder::message('Expected native type must be a literal string.')->nonIgnorable()->identifier('phpstan.unknownExpectation')->build()];
        }
        $expressionType = $scope->getNativeType($args[1]->value)->describe(VerbosityLevel::precise());
        if ($expectedTypeStrings[0]->getValue() === $expressionType) {
            return [];
        }
        return [RuleErrorBuilder::message(sprintf('Expected native type %s, actual: %s', $expectedTypeStrings[0]->getValue(), $expressionType))->nonIgnorable()->identifier('phpstan.nativeType')->build()];
    }
    /**
     * @param Node\Arg[] $args
     * @return list<IdentifierRuleError>
    ...
    private function processAssertVariableCertainty(array $args, Scope $scope): array
    {
        ...
        if (!$certainty instanceof StaticCall) {
            return [RuleErrorBuilder::message('First argument of %s() must be TrinaryLogic call')->nonIgnorable()->identifier('phpstan.unknownExpectation')->build()];
        }
        ...
        if ($variable instanceof Node\Expr\Variable && is_string($variable->name)) {
            ...
            return [RuleErrorBuilder::message('Invalid assertVariableCertainty call.')->nonIgnorable()->identifier('phpstan.unknownExpectation')->build()];
        }
        if ($expectedCertaintyValue->equals($actualCertaintyValue)) {
            return [];
        }
        return [RuleErrorBuilder::message(sprintf('Expected %s certainty %s, actual: %s', $variableDescription, $expectedCertaintyValue->describe(), $actualCertaintyValue->describe()))->nonIgnorable()->identifier('phpstan.variable')->build()];
    }
}

...
final class OverridingConstantRule implements Rule
{
    ...
    private function processSingleConstant(ClassReflection $classReflection, string $constantName) : array
    {
        ...
        if ($prototype->isFinal()) {
            $errors[] = RuleErrorBuilder::message(sprintf('Constant %s::%s overrides final constant %s::%s.', $classReflection->getDisplayName(), $constantReflection->getName(), $prototype->getDeclaringClass()->getDisplayName(), $prototype->getName()))->identifier('classConstant.final')->nonIgnorable()->build();
        }
        if ($prototype->isPublic()) {
            if (!$constantReflection->isPublic()) {
                $errors[] = RuleErrorBuilder::message(sprintf('%s constant %s::%s overriding public constant %s::%s should also be public.', $constantReflection->isPrivate() ? 'Private' : 'Protected', $constantReflection->getDeclaringClass()->getDisplayName(), $constantReflection->getName(), $prototype->getDeclaringClass()->getDisplayName(), $prototype->getName()))->identifier('classConstant.visibility')->nonIgnorable()->build();
            }
        ...
        if ($prototypeNativeType !== null) {
            if ($constantNativeType !== null) {
                ...
                $errors[] = RuleErrorBuilder::message(sprintf('Constant %s::%s overriding constant %s::%s (%s) should also have native type %s.', $constantReflection->getDeclaringClass()->getDisplayName(), $constantReflection->getName(), $prototype->getDeclaringClass()->getDisplayName(), $prototype->getName(), $prototypeNativeType->describe(VerbosityLevel::typeOnly()), $prototypeNativeType->describe(VerbosityLevel::typeOnly())))->identifier('classConstant.missingNativeType')->nonIgnorable()->build();
            }
        }

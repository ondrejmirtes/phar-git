    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        foreach ($classType->getConstantStrings() as $classNameType) {
            ...
            foreach ($templateType->getConstantStrings() as $templateTypeName) {
                ...
                $errors[] = RuleErrorBuilder::message(sprintf('Call to %s::%s() references unknown template type %s on class %s.', $methodReflection->getDeclaringClass()->getDisplayName(), $methodReflection->getName(), $templateTypeName->getValue(), $classReflection->getDisplayName()))->identifier('phpstanApi.getTemplateType')->build();
            }
        }
        return $errors;
    }

...
final class ApiInstanceofTypeRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        if ($lowerMap[$lowerClassName] === null) {
            return [RuleErrorBuilder::message(sprintf('Doing instanceof %s is error-prone and deprecated.', $className))->identifier('phpstanApi.instanceofType')->tip($tip)->build()];
        }
        return [RuleErrorBuilder::message(sprintf('Doing instanceof %s is error-prone and deprecated. Use %s instead.', $className, $lowerMap[$lowerClassName]))->identifier('phpstanApi.instanceofType')->tip($tip)->build()];
    }
}

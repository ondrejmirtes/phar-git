...
{
    private \PHPStan\Rules\Api\ApiRuleHelper $apiRuleHelper;
    ...
    public function processNode(Node $node, Scope $scope) : array
    {
        ...
        foreach ($node->traits as $traitName) {
            ...
            $errors[] = RuleErrorBuilder::message(sprintf('Using %s is not covered by backward compatibility promise. The trait might change in a minor PHPStan version.', $traitReflection->getDisplayName()))->identifier('phpstanApi.trait')->tip($tip)->build();
        }
        return $errors;
    }
}

...
final class AccessStaticPropertiesRule implements Rule
{
    ...
    public function __construct(ReflectionProvider $reflectionProvider, RuleLevelHelper $ruleLevelHelper, ClassNameCheck $classCheck, bool $discoveringSymbolsTip)
    {
    ...
    private function processSingleProperty(Scope $scope, StaticPropertyFetch $node, string $name) : array
    {
        ...
        if ($node->class instanceof Name) {
            ...
            if (in_array($lowercasedClass, ['self', 'static'], \true)) {
                if (!$scope->isInClass()) {
                    return [RuleErrorBuilder::message(sprintf('Accessing %s::$%s outside of class scope.', $class, $name))->identifier(sprintf('outOfClass.%s', $lowercasedClass))->build()];
                }
                $classType = $scope->resolveTypeByName($node->class);
            } elseif ($lowercasedClass === 'parent') {
                if (!$scope->isInClass()) {
                    return [RuleErrorBuilder::message(sprintf('Accessing %s::$%s outside of class scope.', $class, $name))->identifier('outOfClass.parent')->build()];
                }
                if ($scope->getClassReflection()->getParentClass() === null) {
                    return [RuleErrorBuilder::message(sprintf('%s::%s() accesses parent::$%s but %s does not extend any class.', $scope->getClassReflection()->getDisplayName(), $scope->getFunctionName(), $name, $scope->getClassReflection()->getDisplayName()))->identifier('class.noParent')->build()];
                }
                $classType = $scope->resolveTypeByName($node->class);
            } else {
                ...
                $messages = $this->classCheck->checkClassNames($scope, [new ClassNameNodePair($class, $node->class)], ClassNameUsageLocation::from(ClassNameUsageLocation::STATIC_PROPERTY_ACCESS, $locationData));
                $classType = $scope->resolveTypeByName($node->class);
            }
        } else {
        ...
        if ($classType->canAccessProperties()->no() || $classType->canAccessProperties()->maybe() && !$scope->isUndefinedExpressionAllowed($node)) {
            return array_merge($messages, [RuleErrorBuilder::message(sprintf('Cannot access static property $%s on %s.', $name, $typeForDescribe->describe(VerbosityLevel::typeOnly())))->identifier('staticProperty.nonObject')->build()]);
        }
            ...
            if (count($classNames) === 1) {
                ...
                while ($parentClassReflection !== null) {
                    if ($parentClassReflection->hasProperty($name)) {
                        ...
                        return [RuleErrorBuilder::message(sprintf('Access to private static property $%s of parent class %s.', $name, $parentClassReflection->getDisplayName()))->identifier('staticProperty.private')->build()];
                    }
                    $parentClassReflection = $parentClassReflection->getParentClass();
                }
            }
            return array_merge($messages, [RuleErrorBuilder::message(sprintf('Access to an undefined static property %s::$%s.', $typeForDescribe->describe(VerbosityLevel::typeOnly()), $name))->identifier('staticProperty.notFound')->build()]);
        }
        ...
        if (!$property->isStatic()) {
            ...
            return array_merge($messages, [RuleErrorBuilder::message(sprintf('Static access to instance property %s::$%s.', $property->getDeclaringClass()->getDisplayName(), $name))->identifier('property.staticAccess')->build()]);
        }
        if (!$scope->canReadProperty($property)) {
            return array_merge($messages, [RuleErrorBuilder::message(sprintf('Access to %s property $%s of class %s.', $property->isPrivate() ? 'private' : 'protected', $name, $property->getDeclaringClass()->getDisplayName()))->identifier(sprintf('staticProperty.%s', $property->isPrivate() ? 'private' : 'protected'))->build()]);
        }
        return $messages;
    }
}

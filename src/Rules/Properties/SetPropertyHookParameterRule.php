...
final class SetPropertyHookParameterRule implements Rule
{
    ...
    public function __construct(MissingTypehintCheck $missingTypehintCheck, bool $checkPhpDocMethodSignatures, bool $checkMissingTypehints)
    {
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        if (!$propertyReflection->hasNativeType()) {
            if ($parameter->hasNativeType()) {
                $errors[] = RuleErrorBuilder::message(sprintf('Parameter $%s of set hook has a native type but the property %s::$%s does not.', $parameter->getName(), $classReflection->getDisplayName(), $hookReflection->getHookedPropertyName()))->identifier('propertySetHook.nativeParameterType')->nonIgnorable()->build();
            }
        ...
        foreach ($this->missingTypehintCheck->getIterableTypesWithMissingValueTypehint($parameterType) as $iterableType) {
            ...
            $errors[] = RuleErrorBuilder::message(sprintf('Set hook for property %s::$%s has parameter $%s with no value type specified in iterable type %s.', $classReflection->getDisplayName(), $hookReflection->getHookedPropertyName(), $parameter->getName(), $iterableTypeDescription))->tip(MissingTypehintCheck::MISSING_ITERABLE_VALUE_TYPE_TIP)->identifier('missingType.iterableValue')->build();
        }
        foreach ($this->missingTypehintCheck->getNonGenericObjectTypesWithGenericClass($parameterType) as [$name, $genericTypeNames]) {
            $errors[] = RuleErrorBuilder::message(sprintf('Set hook for property %s::$%s has parameter $%s with generic %s but does not specify its types: %s', $classReflection->getDisplayName(), $hookReflection->getHookedPropertyName(), $parameter->getName(), $name, $genericTypeNames))->identifier('missingType.generics')->build();
        }
        foreach ($this->missingTypehintCheck->getCallablesWithMissingSignature($parameterType) as $callableType) {
            $errors[] = RuleErrorBuilder::message(sprintf('Set hook for property %s::$%s has parameter $%s with no signature specified for %s.', $classReflection->getDisplayName(), $hookReflection->getHookedPropertyName(), $parameter->getName(), $callableType->describe(VerbosityLevel::typeOnly())))->identifier('missingType.callable')->build();
        }
        return $errors;
    }
}

...
final class TypesAssignedToPropertiesRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        foreach ($propertyReflections as $propertyReflection) {
            $errors = array_merge($errors, $this->processSingleProperty($propertyReflection, $propertyFetch, $node->getAssignedExpr()));
        }
        return $errors;
    }
    /**
    ...
    private function processSingleProperty(\PHPStan\Rules\Properties\FoundPropertyReflection $propertyReflection, $fetch, Node\Expr $assignedExpr): array
    {
        ...
        if ($fetch instanceof PropertyFetch && $fetch->var instanceof Node\Expr\Variable && is_string($fetch->var->name) && $fetch->var->name === 'this' && $fetch->name instanceof Node\Identifier && $inFunction instanceof PhpMethodFromParserNodeReflection && $inFunction->isPropertyHook() && $inFunction->getHookedPropertyName() === $fetch->name->toString()) {
        ...
        if (!$accepts->result) {
            ...
            return [RuleErrorBuilder::message(sprintf('%s (%s) does not accept %s.', $propertyDescription, $propertyType->describe($verbosityLevel), $assignedValueType->describe($verbosityLevel)))->identifier('assign.propertyType')->acceptsReasonsTip($accepts->reasons)->build()];
        }
        return [];
    }

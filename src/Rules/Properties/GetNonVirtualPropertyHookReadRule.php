...
final class GetNonVirtualPropertyHookReadRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        foreach ($node->getProperties() as $propertyNode) {
            ...
            $errors[] = RuleErrorBuilder::message(sprintf('Get hook for non-virtual property %s::$%s does not read its value.', $classReflection->getDisplayName(), $propertyNode->getName()))->line($this->getGetHookLine($propertyNode))->identifier('propertyGetHook.noRead')->build();
        }
        return $errors;
    }

...
final class AccessPropertiesCheck
{
    ...
    public function __construct(ReflectionProvider $reflectionProvider, RuleLevelHelper $ruleLevelHelper, PhpVersion $phpVersion, bool $reportMagicProperties, bool $checkDynamicProperties)
    {
    ...
    private function processSingleProperty(Scope $scope, PropertyFetch $node, string $name, bool $write): array
    {
        ...
        if ($type->canAccessProperties()->no() || $type->canAccessProperties()->maybe() && !$scope->isUndefinedExpressionAllowed($node)) {
            return [RuleErrorBuilder::message(sprintf('Cannot access property $%s on %s.', $name, $typeForDescribe->describe(VerbosityLevel::typeOnly())))->identifier('property.nonObject')->build()];
        }
            ...
            if (count($classNames) === 1) {
                ...
                while ($parentClassReflection !== null) {
                    if ($parentClassReflection->hasProperty($name)) {
                        ...
                        return [RuleErrorBuilder::message(sprintf('Access to private property $%s of parent class %s.', $name, $parentClassReflection->getDisplayName()))->identifier('property.private')->build()];
                    }
                    $parentClassReflection = $parentClassReflection->getParentClass();
                }
            }
            if ($node->name instanceof Expr) {
                $propertyExistsExpr = new FuncCall(new FullyQualified('property_exists'), [new Arg($node->var), new Arg($node->name)]);
                if ($scope->getType($propertyExistsExpr)->isTrue()->yes()) {
                    return [];
                }
            }
            $ruleErrorBuilder = RuleErrorBuilder::message(sprintf('Access to an undefined property %s::$%s.', $typeForDescribe->describe(VerbosityLevel::typeOnly()), $name))->identifier('property.notFound');
        ...
        if ($propertyReflection->isStatic()) {
            return [RuleErrorBuilder::message(sprintf('Non-static access to static property %s::$%s.', $propertyReflection->getDeclaringClass()->getDisplayName(), $name))->identifier('staticProperty.nonStaticAccess')->build()];
        }
        if ($write) {
        ...
        if (!$this->phpVersion->supportsAsymmetricVisibility() || !$write || !$propertyReflection->isPrivateSet() && !$propertyReflection->isProtectedSet()) {
            return [RuleErrorBuilder::message(sprintf('Access to %s property %s::$%s.', $propertyReflection->isPrivate() ? 'private' : 'protected', $type->describe(VerbosityLevel::typeOnly()), $name))->identifier(sprintf('property.%s', $propertyReflection->isPrivate() ? 'private' : 'protected'))->build()];
        }

...
final class TooWideArrowFunctionReturnTypehintRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        foreach ($functionReturnType->getTypes() as $type) {
            ...
            $messages[] = RuleErrorBuilder::message(sprintf('Anonymous function never returns %s so it can be removed from the return type.', $type->describe(VerbosityLevel::getRecommendedLevelByType($type))))->identifier('return.unusedType')->build();
        }
        return $messages;
    }
}

...
final class TooWidePropertyTypeRule implements Rule
{
    ...
    public function __construct(ReadWritePropertiesExtensionProvider $extensionProvider, PropertyReflectionFinder $propertyReflectionFinder)
    {
    ...
    public function processNode(Node $node, Scope $scope) : array
    {
        ...
        foreach ($node->getProperties() as $property) {
            ...
            foreach ($propertyType->getTypes() as $type) {
                ...
                $errors[] = RuleErrorBuilder::message(sprintf('%s (%s) is never assigned %s so it can be removed from the property type.', $propertyDescription, $propertyType->describe($verbosityLevel), $type->describe($verbosityLevel)))->identifier('property.unusedType')->line($property->getStartLine())->build();
            }
        }
        return $errors;
    }

...
final class TooWideParameterOutTypeCheck
{
    ...
    private function processSingleParameter(Scope $scope, string $functionDescription, ExtendedParameterReflection $parameter) : array
    {
        ...
        foreach ($outType->getTypes() as $type) {
            ...
            $errorBuilder = RuleErrorBuilder::message(sprintf('%s never assigns %s to &$%s so it can be removed from the %s.', $functionDescription, $type->describe(VerbosityLevel::getRecommendedLevelByType($type)), $parameter->getName(), $isParamOutType ? '@param-out type' : 'by-ref type'))->identifier(sprintf('%s.unusedType', $isParamOutType ? 'paramOut' : 'parameterByRef'));
            if (!$isParamOutType) {

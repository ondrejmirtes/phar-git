...
final class DateTimeInstantiationRule implements \PHPStan\Rules\Rule
{
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        if (count($node->getArgs()) === 0 || !in_array($lowerClassName, ['datetime', 'datetimeimmutable'], \true)) {
            return [];
        }
        ...
        foreach ($arg->getConstantStrings() as $constantString) {
            ...
            foreach ($lastErrors['errors'] as $error) {
                $errors[] = \PHPStan\Rules\RuleErrorBuilder::message(sprintf('Instantiating %s with %s produces an error: %s', $lowerClassName === 'datetime' ? 'DateTime' : 'DateTimeImmutable', $dateString, $error))->identifier(sprintf('new.%s', $lowerClassName === 'datetime' ? 'dateTime' : 'dateTimeImmutable'))->build();
            }
        }
        return $errors;
    }

    ...
    private function processComplexType(Node\ComplexType $complexType): array
    {
        ...
        if ($complexType instanceof Node\UnionType) {
            foreach ($complexType->types as $type) {
                ...
                if (in_array($typeString, self::ONLY_STANDALONE_TYPES, \true)) {
                    return [RuleErrorBuilder::message(sprintf('Type %s cannot be part of a union type declaration.', $type->toString()))->line($complexType->getStartLine())->identifier(sprintf('unionType.%s', $typeString))->nonIgnorable()->build()];
                }
            }
            return [];
        }
        if ($complexType->type instanceof Node\Identifier) {
            ...
            if (in_array($complexTypeString, self::ONLY_STANDALONE_TYPES, \true)) {
                return [RuleErrorBuilder::message(sprintf('Type %s cannot be part of a nullable type declaration.', $complexType->type->toString()))->line($complexType->getStartLine())->identifier(sprintf('nullableType.%s', $complexTypeString))->nonIgnorable()->build()];
            }
        }
        return [];
    }

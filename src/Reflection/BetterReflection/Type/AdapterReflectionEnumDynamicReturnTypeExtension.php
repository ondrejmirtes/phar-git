...
final class AdapterReflectionEnumDynamicReturnTypeExtension implements DynamicMethodReturnTypeExtension
{
    ...
    public function getTypeFromMethodCall(MethodReflection $methodReflection, MethodCall $methodCall, Scope $scope) : ?Type
    {
        ...
        if (in_array($methodReflection->getName(), ['getFileName', 'getExtensionName'], \true)) {
            return new UnionType([new IntersectionType([new StringType(), new AccessoryNonEmptyStringType()]), new ConstantBooleanType(\false)]);
        }
        if ($methodReflection->getName() === 'getDocComment') {
            return new UnionType([new StringType(), new ConstantBooleanType(\false)]);
        }
        if (in_array($methodReflection->getName(), ['getStartLine', 'getEndLine'], \true)) {
            return new IntegerType();
        }
        if ($methodReflection->getName() === 'getReflectionConstant') {
            return new UnionType([new ObjectType(ReflectionClassConstant::class), new ConstantBooleanType(\false)]);
        }
        if ($methodReflection->getName() === 'getParentClass') {
            return new UnionType([new ObjectType(ReflectionClass::class), new ConstantBooleanType(\false)]);
        }
        if ($methodReflection->getName() === 'getBackingType') {
            return new UnionType([new ObjectType(ReflectionNamedType::class), new NullType()]);
        }
        return null;
    }
}

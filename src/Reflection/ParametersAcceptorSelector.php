...
final class ParametersAcceptorSelector
{
    ...
    public static function selectFromArgs(Scope $scope, array $args, array $parametersAcceptors, ?array $namedArgumentsVariants = null): \PHPStan\Reflection\ParametersAcceptor
    {
        ...
        if (count($args) > 0 && count($parametersAcceptors) > 0) {
            ...
            if ($arrayMapArgs !== null) {
                        ...
                        if (count($constantArrays) > 0) {
                            foreach ($constantArrays as $constantArray) {
                                ...
                                foreach ($valueTypes as $valueType) {
                                    $callbackParameters[] = new DummyParameter('item', $scope->getIterableValueType($valueType), \false, \PHPStan\Reflection\PassedByReference::createNo(), \false, null);
                                }
                            }
                        }
                    } else {
                        $callbackParameters[] = new DummyParameter('item', $scope->getIterableValueType($argType), \false, \PHPStan\Reflection\PassedByReference::createNo(), \false, null);
                    }
                }
                ...
                $parametersAcceptors = [new \PHPStan\Reflection\FunctionVariant($acceptor->getTemplateTypeMap(), $acceptor->getResolvedTemplateTypeMap(), $parameters, $acceptor->isVariadic(), $acceptor->getReturnType(), $acceptor instanceof \PHPStan\Reflection\ExtendedParametersAcceptor ? $acceptor->getCallSiteVarianceMap() : TemplateTypeVarianceMap::createEmpty())];
            }
            if (count($args) >= 3 && (bool) $args[0]->getAttribute(CurlSetOptArgVisitor::ATTRIBUTE_NAME)) {
                ...
                if ($optType instanceof ConstantIntegerType) {
                    ...
                    if ($optValueType !== null) {
                        ...
                        $parametersAcceptors = [new \PHPStan\Reflection\FunctionVariant($acceptor->getTemplateTypeMap(), $acceptor->getResolvedTemplateTypeMap(), array_values($parameters), $acceptor->isVariadic(), $acceptor->getReturnType(), $acceptor instanceof \PHPStan\Reflection\ExtendedParametersAcceptor ? $acceptor->getCallSiteVarianceMap() : TemplateTypeVarianceMap::createEmpty())];
                    }
                }
            }
            ...
            if (isset($args[0]) && (bool) $args[0]->getAttribute(ArrayFindArgVisitor::ATTRIBUTE_NAME)) {
                ...
                $parameters[1] = new NativeParameterReflection($parameters[1]->getName(), $parameters[1]->isOptional(), new CallableType([new DummyParameter('value', $scope->getIterableValueType($argType), \false, \PHPStan\Reflection\PassedByReference::createNo(), \false, null), new DummyParameter('key', $scope->getIterableKeyType($argType), \false, \PHPStan\Reflection\PassedByReference::createNo(), \false, null)], new BooleanType(), \false), $parameters[1]->passedByReference(), $parameters[1]->isVariadic(), $parameters[1]->getDefaultValue());
                $parametersAcceptors = [new \PHPStan\Reflection\FunctionVariant($acceptor->getTemplateTypeMap(), $acceptor->getResolvedTemplateTypeMap(), array_values($parameters), $acceptor->isVariadic(), $acceptor->getReturnType(), $acceptor instanceof \PHPStan\Reflection\ExtendedParametersAcceptor ? $acceptor->getCallSiteVarianceMap() : TemplateTypeVarianceMap::createEmpty())];
            }
            if (isset($args[0])) {
                $closureBindToVar = $args[0]->getAttribute(ClosureBindToVarVisitor::ATTRIBUTE_NAME);
                if ($closureBindToVar !== null && $closureBindToVar instanceof Node\Expr\Variable && is_string($closureBindToVar->name)) {
                        ...
                        if ($inFunction !== null) {
                            ...
                            if (array_key_exists($closureBindToVar->name, $closureThisParameters)) {
                                if ($scope->hasExpressionType(new ParameterVariableOriginalValueExpr($closureBindToVar->name))->yes()) {
                                    $acceptor = $parametersAcceptors[0];
                                    $parameters = $acceptor->getParameters();
                                    $parameters[0] = new NativeParameterReflection($parameters[0]->getName(), $parameters[0]->isOptional(), $closureThisParameters[$closureBindToVar->name], $parameters[0]->passedByReference(), $parameters[0]->isVariadic(), $parameters[0]->getDefaultValue());
                                    $parametersAcceptors = [new \PHPStan\Reflection\FunctionVariant($acceptor->getTemplateTypeMap(), $acceptor->getResolvedTemplateTypeMap(), $parameters, $acceptor->isVariadic(), $acceptor->getReturnType(), $acceptor instanceof \PHPStan\Reflection\ExtendedParametersAcceptor ? $acceptor->getCallSiteVarianceMap() : TemplateTypeVarianceMap::createEmpty())];
                                }
                            }
                        }
                ...
                if ($args[0]->getAttribute(ClosureBindArgVisitor::ATTRIBUTE_NAME) !== null && $args[0]->value instanceof Node\Expr\Variable && is_string($args[0]->value->name)) {
                    ...
                    if ($inFunction !== null) {
                        ...
                        if (array_key_exists($closureVarName, $closureThisParameters)) {
                            if ($scope->hasExpressionType(new ParameterVariableOriginalValueExpr($closureVarName))->yes()) {
                                $acceptor = $parametersAcceptors[0];
                                $parameters = $acceptor->getParameters();
                                $parameters[1] = new NativeParameterReflection($parameters[1]->getName(), $parameters[1]->isOptional(), $closureThisParameters[$closureVarName], $parameters[1]->passedByReference(), $parameters[1]->isVariadic(), $parameters[1]->getDefaultValue());
                                $parametersAcceptors = [new \PHPStan\Reflection\FunctionVariant($acceptor->getTemplateTypeMap(), $acceptor->getResolvedTemplateTypeMap(), array_values($parameters), $acceptor->isVariadic(), $acceptor->getReturnType(), $acceptor instanceof \PHPStan\Reflection\ExtendedParametersAcceptor ? $acceptor->getCallSiteVarianceMap() : TemplateTypeVarianceMap::createEmpty())];
                            }
                        }
                    }
    ...
    private static function hasAcceptorTemplateOrLateResolvableType(\PHPStan\Reflection\ParametersAcceptor $acceptor): bool
    {
        ...
        foreach ($acceptor->getParameters() as $parameter) {
            if ($parameter instanceof \PHPStan\Reflection\ExtendedParameterReflection && $parameter->getOutType() !== null && self::hasTemplateOrLateResolvableType($parameter->getOutType())) {
                return \true;
            }
            if ($parameter instanceof \PHPStan\Reflection\ExtendedParameterReflection && $parameter->getClosureThisType() !== null && self::hasTemplateOrLateResolvableType($parameter->getClosureThisType())) {
                return \true;
            }
    ...
    public static function combineAcceptors(array $acceptors): \PHPStan\Reflection\ExtendedParametersAcceptor
    {
        ...
        foreach ($acceptors as $acceptor) {
            ...
            foreach ($acceptor->getParameters() as $i => $parameter) {
                if (!isset($parameters[$i])) {
                    $parameters[$i] = new ExtendedDummyParameter($parameter->getName(), $parameter->getType(), $i + 1 > $minimumNumberOfParameters, $parameter->passedByReference(), $parameter->isVariadic(), $parameter->getDefaultValue(), $parameter instanceof \PHPStan\Reflection\ExtendedParameterReflection ? $parameter->getNativeType() : new MixedType(), $parameter instanceof \PHPStan\Reflection\ExtendedParameterReflection ? $parameter->getPhpDocType() : new MixedType(), $parameter instanceof \PHPStan\Reflection\ExtendedParameterReflection ? $parameter->getOutType() : null, $parameter instanceof \PHPStan\Reflection\ExtendedParameterReflection ? $parameter->isImmediatelyInvokedCallable() : TrinaryLogic::createMaybe(), $parameter instanceof \PHPStan\Reflection\ExtendedParameterReflection ? $parameter->getClosureThisType() : null, $parameter instanceof \PHPStan\Reflection\ExtendedParameterReflection ? $parameter->getAttributes() : []);
                    continue;
                }
                ...
                if ($parameter instanceof \PHPStan\Reflection\ExtendedParameterReflection) {
                ...
                $parameters[$i] = new ExtendedDummyParameter($parameters[$i]->getName() !== $parameter->getName() ? sprintf('%s|%s', $parameters[$i]->getName(), $parameter->getName()) : $parameter->getName(), $type, $i + 1 > $minimumNumberOfParameters, $parameters[$i]->passedByReference()->combine($parameter->passedByReference()), $isVariadic, $defaultValue, $nativeType, $phpDocType, $outType, $immediatelyInvokedCallable, $closureThisType, $attributes);
                if ($isVariadic) {
        ...
        if ($callableOccurred) {
            return new \PHPStan\Reflection\ExtendedCallableFunctionVariant(TemplateTypeMap::createEmpty(), null, array_values($parameters), $isVariadic, $returnType, $phpDocReturnType ?? $returnType, $nativeReturnType ?? new MixedType(), null, $throwPoints, $isPure, $impurePoints, $invalidateExpressions, $usedVariables, $acceptsNamedArguments);
        }
    ...
    private static function wrapAcceptor(\PHPStan\Reflection\ParametersAcceptor $acceptor): \PHPStan\Reflection\ExtendedParametersAcceptor
    {
        ...
        if ($acceptor instanceof CallableParametersAcceptor) {
            return new \PHPStan\Reflection\ExtendedCallableFunctionVariant($acceptor->getTemplateTypeMap(), $acceptor->getResolvedTemplateTypeMap(), array_map(static fn(\PHPStan\Reflection\ParameterReflection $parameter): \PHPStan\Reflection\ExtendedParameterReflection => self::wrapParameter($parameter), $acceptor->getParameters()), $acceptor->isVariadic(), $acceptor->getReturnType(), $acceptor->getReturnType(), new MixedType(), TemplateTypeVarianceMap::createEmpty(), $acceptor->getThrowPoints(), $acceptor->isPure(), $acceptor->getImpurePoints(), $acceptor->getInvalidateExpressions(), $acceptor->getUsedVariables(), $acceptor->acceptsNamedArguments());
        }

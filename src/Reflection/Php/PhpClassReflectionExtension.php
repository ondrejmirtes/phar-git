...
final class PhpClassReflectionExtension implements PropertiesClassReflectionExtension, MethodsClassReflectionExtension
{
    ...
    private function createProperty(ClassReflection $classReflection, string $propertyName, bool $includingAnnotations): ExtendedPropertyReflection
    {
        ...
        if ($declaringClassReflection->isEnum()) {
            if ($propertyName === 'name' || $declaringClassReflection->isBackedEnum() && $propertyName === 'value') {
                ...
                return new \PHPStan\Reflection\Php\PhpPropertyReflection($declaringClassReflection, null, null, TypeCombinator::union(...$types), $classReflection->getNativeReflection()->getProperty($propertyName), null, null, null, \false, \false, \false, \false, [], \false);
            }
        }
        ...
        if ($betterReflection->hasHook('get')) {
            ...
            $getHook = $this->createUserlandMethodReflection($declaringClassReflection, $declaringClassReflection, new ReflectionMethod($betterReflectionGetHook), $declaringTraitName);
            if ($phpDocType !== null) {
                ...
                if ($getHookMethodReflectionVariantPhpDocReturnType instanceof MixedType && !$getHookMethodReflectionVariantPhpDocReturnType instanceof TemplateMixedType && !$getHookMethodReflectionVariantPhpDocReturnType->isExplicitMixed()) {
        ...
        if ($betterReflection->hasHook('set')) {
            ...
            $setHook = $this->createUserlandMethodReflection($declaringClassReflection, $declaringClassReflection, new ReflectionMethod($betterReflectionSetHook), $declaringTraitName);
            if ($phpDocType !== null) {
                ...
                if (isset($setHookMethodReflectionParameters[0])) {
                    ...
                    if ($setHookMethodReflectionParameterPhpDocType instanceof MixedType && !$setHookMethodReflectionParameterPhpDocType instanceof TemplateMixedType && !$setHookMethodReflectionParameterPhpDocType->isExplicitMixed()) {
            ...
            foreach ($methodSignaturesResult as $signatureType => $methodSignatures) {
                ...
                foreach ($methodSignatures as $methodSignature) {
                    ...
                    if ($stubPhpDocPair === null && $methodReflection->getDocComment() !== \false) {
                        ...
                        if ($filename !== \false) {
                            $phpDocBlock = $this->fileTypeMapper->getResolvedPhpDoc($filename, $declaringClassName, null, $methodReflection->getName(), $methodReflection->getDocComment());
    ...
    public function createUserlandMethodReflection(ClassReflection $fileDeclaringClass, ClassReflection $actualDeclaringClass, ReflectionMethod $methodReflection, ?string $declaringTraitName): \PHPStan\Reflection\Php\PhpMethodReflection
    {
        ...
        if ($resolvedPhpDoc === null) {
            ...
            $resolvedPhpDoc = $this->phpDocInheritanceResolver->resolvePhpDocForMethod($docComment, $actualDeclaringClass->getFileName(), $actualDeclaringClass, $declaringTraitName, $methodReflection->getName(), $positionalParameterNames);
            $phpDocBlockClassReflection = $fileDeclaringClass;
        }
        $declaringTrait = null;
        $reflectionProvider = $this->reflectionProviderProvider->getReflectionProvider();
        ...
        if ($methodReflection->isConstructor()) {
            foreach ($methodReflection->getParameters() as $parameter) {
                ...
                $propertyDocblock = $this->fileTypeMapper->getResolvedPhpDoc($fileDeclaringClass->getFileName(), $fileDeclaringClass->getName(), $declaringTraitName, $methodReflection->getName(), $parameterProperty->getDocComment());
        ...
        $nativeReturnType = TypehintHelper::decideTypeFromReflection($methodReflection->getReturnType(), null, $actualDeclaringClass);
    ...
    private function createNativeMethodVariant(FunctionSignature $methodSignature, array $stubPhpDocParameterTypes, array $stubPhpDocParameterVariadicity, ?Type $stubPhpDocReturnType, array $phpDocParameterTypes, ?Type $phpDocReturnType, array $phpDocParameterNameMapping, array $stubPhpDocParameterOutTypes, array $phpDocParameterOutTypes, array $stubImmediatelyInvokedCallableParameters, array $immediatelyInvokedCallableParameters, array $stubClosureThisParameters, array $closureThisParameters, bool $usePhpDocParameterNames): ExtendedFunctionVariant
    {
        ...
        foreach ($methodSignature->getParameters() as $parameterSignature) {
            ...
            $parameters[] = new ExtendedNativeParameterReflection($usePhpDocParameterNames ? $phpDocParameterName : $parameterSignature->getName(), $parameterSignature->isOptional(), $type ?? $parameterSignature->getType(), $phpDocType ?? new MixedType(), $parameterSignature->getNativeType(), $parameterSignature->passedByReference(), $stubPhpDocParameterVariadicity[$parameterSignature->getName()] ?? $parameterSignature->isVariadic(), $parameterSignature->getDefaultValue(), $parameterOutType ?? $parameterSignature->getOutType(), $immediatelyInvoked, $closureThisType, []);
        }
    ...
    private function inferAndCachePropertyTypes(MethodReflection $constructor): array
    {
        ...
        $methodScope = $classScope->enterClassMethod($methodNode, $templateTypeMap, $phpDocParameterTypes, $phpDocReturnType, $phpDocThrowType, $deprecatedDescription, $isDeprecated, $isInternal, $isFinal, $isPure, $acceptsNamedArguments, $asserts, $selfOutType, $phpDocComment, $phpDocParameterOutTypes, $phpDocImmediatelyInvokedCallableParameters, $phpDocClosureThisTypeParameters);
        $propertyTypes = [];

...
final class PHPStanDiagnoseExtension implements \PHPStan\Diagnose\DiagnoseExtension
{
     ...
     */
    public function __construct(PhpVersion $phpVersion, $configPhpVersion, FileHelper $fileHelper, array $composerAutoloaderProjectPaths, array $allConfigFiles, ComposerPhpVersionFactory $composerPhpVersionFactory)
    {
    ...
    public function print(Output $output): void
    {
        ...
        if ($this->phpVersion->getSource() === PhpVersion::SOURCE_CONFIG && is_array($this->configPhpVersion)) {
            $minVersion = new PhpVersion($this->configPhpVersion['min']);
            $maxVersion = new PhpVersion($this->configPhpVersion['max']);
            $output->writeLineFormatted(sprintf('<info>PHP version for analysis:</info> %s-%s (from %s)', $minVersion->getVersionString(), $maxVersion->getVersionString(), $this->phpVersion->getSourceLabel()));
        } else {
            ...
            if ($minComposerPhpVersion !== null && $maxComposerPhpVersion !== null) {
                if ($minComposerPhpVersion->getVersionId() !== $maxComposerPhpVersion->getVersionId()) {
                    $output->writeLineFormatted(sprintf('<info>PHP composer.json required version:</info> %s-%s', $minComposerPhpVersion->getVersionString(), $maxComposerPhpVersion->getVersionString()));
                } else {
                    $output->writeLineFormatted(sprintf('<info>PHP composer.json required version:</info> %s', $minComposerPhpVersion->getVersionString()));
                }
            }
            $output->writeLineFormatted(sprintf('<info>PHP version for analysis:</info> %s (from %s)', $this->phpVersion->getVersionString(), $this->phpVersion->getSourceLabel()));
        }
        $output->writeLineFormatted('');
        $output->writeLineFormatted(sprintf('<info>PHPStan version:</info> %s', ComposerHelper::getPhpStanVersion()));
        ...
        foreach ($this->allConfigFiles as $configFile) {
            ...
            if (in_array($configFile, $configFilesFromExtensionInstaller, \true)) {
                continue;
            }
        ...
        $output->writeLineFormatted(sprintf('<info>Discovered Composer project %s:</info>', $composerAutoloaderProjectPathsCount === 1 ? 'root' : 'roots'));

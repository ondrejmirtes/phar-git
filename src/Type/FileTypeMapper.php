...
final class FileTypeMapper
{
     ...
     */
    public function getNameScope(string $fileName, ?string $className, ?string $traitName, ?string $functionName) : NameScope
    {
        $nameScopeKey = $this->getNameScopeKey($fileName, $className, $traitName, $functionName);
        $nameScopeMap = [];
    ...
    private function createResolvedPhpDocBlock(string $phpDocKey, NameScope $nameScope, string $phpDocString, ?string $fileName) : ResolvedPhpDocBlock
    {
        ...
        $this->resolvedPhpDocBlockCache[$phpDocKey] = ResolvedPhpDocBlock::create($phpDocNode, $phpDocString, $fileName, $nameScope, new TemplateTypeMap($phpDocTemplateTypes), $templateTags, $this->phpDocNodeResolver, $this->reflectionProviderProvider->getReflectionProvider());
        $this->resolvedPhpDocBlockCacheCount++;
        return $this->resolvedPhpDocBlockCache[$phpDocKey];
    }
    /**
    ...
    private function createNameScopeMap(string $fileName, ?string $lookForTrait, ?string $traitUseClass, array $traitMethodAliases, string $originalClassFileName, array $phpDocNodeMap) : array
    {
        ...
        $this->processNodes($this->phpParser->parseFile($fileName), function (Node $node) use($fileName, $lookForTrait, $phpDocNodeMap, &$traitFound, $traitMethodAliases, $originalClassFileName, &$nameScopeMap, &$classStack, &$typeAliasStack, &$namespace, &$functionStack, &$uses, &$typeMapStack, &$constUses) : ?int {
            ...
            if ($node instanceof Node\Stmt\ClassLike || $node instanceof Node\Stmt\ClassMethod || $node instanceof Node\Stmt\Function_) {
                ...
                if (array_key_exists($nameScopeKey, $phpDocNodeMap)) {
                    ...
                    $typeMapStack[] = function () use($namespace, $uses, $className, $lookForTrait, $functionName, $phpDocNode, $typeMapStack, $typeAliasStack, $constUses) : TemplateTypeMap {
                        ...
                        $templateTypeMap = new TemplateTypeMap(array_map(static fn(TemplateTag $tag): \PHPStan\Type\Type => TemplateTypeFactory::fromTemplateTag($templateTypeScope, $tag), $templateTags));
            ...
            if (($node instanceof Node\PropertyHook || $node instanceof Node\Stmt && !$node instanceof Node\Stmt\Namespace_ && !$node instanceof Node\Stmt\Declare_ && !$node instanceof Node\Stmt\Use_ && !$node instanceof Node\Stmt\GroupUse && !$node instanceof Node\Stmt\TraitUse && !$node instanceof Node\Stmt\TraitUseAdaptation && !$node instanceof Node\Stmt\InlineHTML && !($node instanceof Node\Stmt\Expression && $node->expr instanceof Node\Expr\Include_)) && !array_key_exists($nameScopeKey, $nameScopeMap)) {
                $nameScopeMap[$nameScopeKey] = static fn(): NameScope => new NameScope($namespace, $uses, $className, $functionName, $typeMapCb !== null ? $typeMapCb() : TemplateTypeMap::createEmpty(), $typeAliasesMap, \false, $constUses, $lookForTrait);
            }
            if ($node instanceof Node\Stmt\ClassLike || $node instanceof Node\Stmt\ClassMethod || $node instanceof Node\Stmt\Function_) {
                // property hook skipped on purpose, it does not support @template
                if (array_key_exists($nameScopeKey, $phpDocNodeMap)) {
                ...
                foreach ($node->traits as $traitName) {
                    ...
                    foreach ($traitPhpDocMap as $nameScopeTraitKey => $callback) {
                        $finalTraitPhpDocMap[$nameScopeTraitKey] = function () use($callback, $traitReflection, $fileName, $className, $lookForTrait, $useDocComment) : NameScope {
                            ...
                            return $original->withTemplateTypeMap($traitTemplateTypeMap->map(static fn(string $name, \PHPStan\Type\Type $type): \PHPStan\Type\Type => TemplateTypeHelper::resolveTemplateTypes($type, $transformedTraitTypeMap, TemplateTypeVarianceMap::createEmpty(), TemplateTypeVariance::createStatic())));
                        };
                    }

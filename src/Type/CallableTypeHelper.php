...
final class CallableTypeHelper
{
    public static function isParametersAcceptorSuperTypeOf(CallableParametersAcceptor $ours, CallableParametersAcceptor $theirs, bool $treatMixedAsAny): \PHPStan\Type\IsSuperTypeOfResult
    {
        ...
        if ($lastParameter !== null && $lastParameter->isVariadic() && $theirParameterCount < $ourParameterCount) {
        ...
        foreach ($theirParameters as $i => $theirParameter) {
            ...
            if (!isset($ourParameters[$i])) {
                ...
                $accepts = new \PHPStan\Type\IsSuperTypeOfResult(TrinaryLogic::createNo(), [sprintf('Parameter %s of passed callable is required but accepting callable does not have that parameter. It will be called without it.', $parameterDescription)]);
            ...
            if ($ourParameter->isOptional() && !$theirParameter->isOptional()) {
                $accepts = new \PHPStan\Type\IsSuperTypeOfResult(TrinaryLogic::createNo(), [sprintf('Parameter %s of passed callable is required but the parameter of accepting callable is optional. It might be called without it.', $parameterDescription)]);
                $result = $result->and($accepts);
            }
            if ($treatMixedAsAny) {
                $isSuperType = $theirParameter->getType()->accepts($ourParameterType, \true);
                $isSuperType = new \PHPStan\Type\IsSuperTypeOfResult($isSuperType->result, $isSuperType->reasons);
            } else {
            ...
            if ($isSuperType->maybe()) {
                ...
                $isSuperType = new \PHPStan\Type\IsSuperTypeOfResult($isSuperType->result, array_merge($isSuperType->reasons, [sprintf('Type %s of parameter %s of passed callable needs to be same or wider than parameter type %s of accepting callable.', $theirParameter->getType()->describe($verbosity), $parameterDescription, $ourParameterType->describe($verbosity))]));
            }
            $result = $result->and($isSuperType);
        }
        if (!$treatMixedAsAny && $theirParameterCount < $ourParameterCount) {
            $result = $result->and(\PHPStan\Type\IsSuperTypeOfResult::createMaybe());
        }
        $theirReturnType = $theirs->getReturnType();
        if ($treatMixedAsAny) {
            $isReturnTypeSuperType = $ours->getReturnType()->accepts($theirReturnType, \true);
            $isReturnTypeSuperType = new \PHPStan\Type\IsSuperTypeOfResult($isReturnTypeSuperType->result, $isReturnTypeSuperType->reasons);
        } else {
            $isReturnTypeSuperType = $ours->getReturnType()->isSuperTypeOf($theirReturnType);
        }
        $pure = $ours->isPure();
        if ($pure->yes()) {
            $result = $result->and(new \PHPStan\Type\IsSuperTypeOfResult($theirs->isPure(), []));
        } elseif ($pure->no()) {
            $result = $result->and(new \PHPStan\Type\IsSuperTypeOfResult($theirs->isPure()->negate(), []));
        }

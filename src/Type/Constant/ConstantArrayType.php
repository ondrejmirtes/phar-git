...
class ConstantArrayType implements Type
{
     ...
     */
    public function __construct(array $keyTypes, array $valueTypes, array $nextAutoIndexes = [0], array $optionalKeys = [], ?TrinaryLogic $isList = null)
    {
    ...
    public function accepts(Type $type, bool $strictTypes): AcceptsResult
    {
        ...
        foreach ($this->keyTypes as $i => $keyType) {
            ...
            $acceptsValue = $valueType->accepts($otherValueType, $strictTypes)->decorateReasons(static fn(string $reason) => sprintf('Offset %s (%s) does not accept type %s: %s', $keyType->describe(VerbosityLevel::precise()), $valueType->describe($verbosity), $otherValueType->describe($verbosity), $reason));
            if (!$acceptsValue->yes() && count($acceptsValue->reasons) === 0 && $type->isConstantArray()->yes()) {
                $acceptsValue = new AcceptsResult($acceptsValue->result, [sprintf('Offset %s (%s) does not accept type %s.', $keyType->describe(VerbosityLevel::precise()), $valueType->describe($verbosity), $otherValueType->describe($verbosity))]);
            }
            if ($acceptsValue->no()) {
                return $acceptsValue;
            }
        ...
        if ($this->isIterableAtLeastOnce()->no()) {
            ...
            if (count($constantScalarValues) > 0) {
                ...
                foreach ($constantScalarValues as $constantScalarValue) {
                    ...
                    $results[] = TrinaryLogic::createFromBoolean($constantScalarValue == []);
                    // phpcs:ignore
                }
    ...
    public function findTypeAndMethodNames(): array
    {
        ...
        $method = null;
        foreach ($this->keyTypes as $i => $keyType) {
            if ($keyType->isSuperTypeOf(new \PHPStan\Type\Constant\ConstantIntegerType(0))->yes()) {
                $classOrObject = $this->valueTypes[$i];
                continue;
            }
            if (!$keyType->isSuperTypeOf(new \PHPStan\Type\Constant\ConstantIntegerType(1))->yes()) {
                continue;
            }
        ...
        foreach ($methods->getConstantStrings() as $methodName) {
            ...
            $typeAndMethods[] = \PHPStan\Type\Constant\ConstantArrayTypeAndMethod::createConcrete($type, $methodName->getValue(), $has);
        }
    ...
    private function recursiveHasOffsetValueType(Type $offsetType): TrinaryLogic
    {
        ...
        foreach ($this->keyTypes as $i => $keyType) {
            if ($keyType instanceof \PHPStan\Type\Constant\ConstantIntegerType && !$offsetType->isString()->no() && $offsetType->isConstantScalarValue()->no()) {
    ...
    public function chunkArray(Type $lengthType, TrinaryLogic $preserveKeys): Type
    {
        ...
        if ($biggerOne->isSuperTypeOf($lengthType)->yes() && count($finiteTypes) < self::CHUNK_FINITE_TYPES_LIMIT) {
            ...
            foreach ($finiteTypes as $finiteType) {
                if (!$finiteType instanceof \PHPStan\Type\Constant\ConstantIntegerType || $finiteType->getValue() < 1) {
                ...
                for ($i = 0; $i < $keyTypesCount; $i += $length) {
                    $chunk = $this->sliceArray(new \PHPStan\Type\Constant\ConstantIntegerType($i), new \PHPStan\Type\Constant\ConstantIntegerType($length), TrinaryLogic::createYes());
    ...
    public function reverseArray(TrinaryLogic $preserveKeys): Type
    {
        ...
        for ($i = count($this->keyTypes) - 1; $i >= 0; $i--) {
            $offsetType = $preserveKeys->yes() || $this->keyTypes[$i]->isInteger()->no() ? $this->keyTypes[$i] : null;
    ...
    public function sliceArray(Type $offsetType, Type $lengthType, TrinaryLogic $preserveKeys): Type
    {
        ...
        $offset = $offsetType instanceof \PHPStan\Type\Constant\ConstantIntegerType ? $offsetType->getValue() : null;
        if ($lengthType instanceof \PHPStan\Type\Constant\ConstantIntegerType) {
        ...
        if ($offset === null || $length === null) {
            $builder = \PHPStan\Type\Constant\ConstantArrayTypeBuilder::createFromConstantArray($this);
            $builder->degradeToGeneralArray();
            return $builder->getArray()->sliceArray($offsetType, $lengthType, $preserveKeys);
        }
        ...
        if ($length < 0) {
            ...
            return $this->removeLastElements($length * -1)->sliceArray($offsetType, new NullType(), $preserveKeys);
        }
        if ($offset < 0) {
            /*
            ...
            return $this->reverseArray(TrinaryLogic::createYes())->sliceArray(new \PHPStan\Type\Constant\ConstantIntegerType($reversedOffset), new \PHPStan\Type\Constant\ConstantIntegerType($reversedLength), $preserveKeys)->reverseArray(TrinaryLogic::createYes());
        }
        if ($offset > 0) {
            return $this->removeFirstElements($offset, \false)->sliceArray(new \PHPStan\Type\Constant\ConstantIntegerType(0), $lengthType, $preserveKeys);
        }
        $builder = \PHPStan\Type\Constant\ConstantArrayTypeBuilder::createEmpty();
    ...
    private function removeLastElements(int $length): self
    {
        ...
        for ($i = $keyTypesCount - 1; $i >= 0; $i--) {
            ...
            if ($i >= $newLength) {
                ...
                $nextAutoindexes = $removedKeyType instanceof \PHPStan\Type\Constant\ConstantIntegerType ? [$removedKeyType->getValue()] : $this->nextAutoIndexes;
                continue;
            }
    ...
    public function generalize(GeneralizePrecision $precision): Type
    {
        ...
        $arrayType = new ArrayType($this->getIterableKeyType()->generalize($precision), $this->getIterableValueType()->generalize($precision));
        $keyTypesCount = count($this->keyTypes);
        $optionalKeysCount = count($this->optionalKeys);
        $accessoryTypes = [];
    ...
    private function getKeysOrValuesArray(array $types): self
    {
        ...
        if ($this->isList->yes()) {
            ...
            $keyTypes = array_map(static fn(int $i): \PHPStan\Type\Constant\ConstantIntegerType => new \PHPStan\Type\Constant\ConstantIntegerType($i), array_keys($types));

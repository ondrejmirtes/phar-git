        ...
        for ($i = 0; $i < count($items); $i++) {
            ...
            if ($valueType instanceof \PHPStan\Type\Constant\ConstantArrayType) {
                ...
                foreach ($valueType->getKeyTypes() as $j => $innerKeyType) {
                    ...
                    array_splice($items, $i++, 0, [new ArrayItem(new TypeExpr($innerValueType), $keyExpr)]);
                }
            } else {
                array_splice($items, $i, 1, [new ArrayItem(new TypeExpr($valueType->getIterableValueType()), new TypeExpr($valueType->getIterableKeyType()))]);
            }
        }

...
final class ParseUrlFunctionDynamicReturnTypeExtension implements DynamicFunctionReturnTypeExtension
{
    ...
    public function getTypeFromFunctionCall(FunctionReflection $functionReflection, FuncCall $functionCall, Scope $scope): ?Type
    {
        ...
        if ($componentType->getValue() === -1) {
            return TypeCombinator::union($this->createComponentsArray($urlType->isLowercaseString()->yes()), new ConstantBooleanType(\false));
        }
    ...
    private function createAllComponentsReturnType(bool $urlIsLowercase): Type
    {
        if ($urlIsLowercase) {
            if ($this->allComponentsTogetherTypeForLowercaseString === null) {
                $returnTypes = [new ConstantBooleanType(\false), new NullType(), IntegerRangeType::fromInterval(0, 65535), new IntersectionType([new StringType(), new AccessoryLowercaseStringType()]), $this->createComponentsArray(\true)];
        ...
        if ($this->allComponentsTogetherType === null) {
            $returnTypes = [new ConstantBooleanType(\false), new NullType(), IntegerRangeType::fromInterval(0, 65535), new StringType(), $this->createComponentsArray(\false)];
    ...
    private function cacheReturnTypes(): void
    {
        ...
        $this->componentTypesPairedConstantsForLowercaseString = [PHP_URL_SCHEME => $lowercaseStringOrFalseOrNull, PHP_URL_HOST => $lowercaseStringOrFalseOrNull, PHP_URL_PORT => $portOrFalseOrNull, PHP_URL_USER => $lowercaseStringOrFalseOrNull, PHP_URL_PASS => $lowercaseStringOrFalseOrNull, PHP_URL_PATH => $lowercaseStringOrFalseOrNull, PHP_URL_QUERY => $lowercaseStringOrFalseOrNull, PHP_URL_FRAGMENT => $lowercaseStringOrFalseOrNull];
        ...
        $this->componentTypesPairedStringsForLowercaseString = ['scheme' => $lowercaseString, 'host' => $lowercaseString, 'port' => $port, 'user' => $lowercaseString, 'pass' => $lowercaseString, 'path' => $lowercaseString, 'query' => $lowercaseString, 'fragment' => $lowercaseString];
    }
}

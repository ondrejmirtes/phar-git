...
final class ArrayKeyExistsFunctionTypeSpecifyingExtension implements FunctionTypeSpecifyingExtension, TypeSpecifierAwareExtension
{
    ...
    public function specifyTypes(FunctionReflection $functionReflection, FuncCall $node, Scope $scope, TypeSpecifierContext $context) : SpecifiedTypes
    {
        ...
        if (!$keyType instanceof ConstantIntegerType && !$keyType instanceof ConstantStringType) {
            if ($context->true()) {
                if ($arrayType->isIterableAtLeastOnce()->no()) {
                    return $this->typeSpecifier->create($array, new NonEmptyArrayType(), $context, $scope);
                }
                ...
                $specifiedTypes = $this->typeSpecifier->create($key, $arrayKeyType, $context, $scope);
                $arrayDimFetch = new ArrayDimFetch($array, $key);
                return $specifiedTypes->unionWith($this->typeSpecifier->create($arrayDimFetch, $arrayType->getIterableValueType(), $context, $scope))->setRootExpr(new Identical($arrayDimFetch, new ConstFetch(new Name('__PHPSTAN_FAUX_CONSTANT'))));
            }
            return new SpecifiedTypes();
        }
        if ($context->true()) {
        ...
        return $this->typeSpecifier->create($array, $type, $context, $scope);
    }
}

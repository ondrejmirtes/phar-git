...
final class VersionCompareFunctionDynamicReturnTypeExtension implements DynamicFunctionReturnTypeExtension
{
     ...
     */
    public function __construct($configPhpVersion, ComposerPhpVersionFactory $composerPhpVersionFactory)
    {
    ...
    private function getVersionStrings(Expr $expr, Scope $scope): array
    {
        if ($expr instanceof Expr\ConstFetch && $expr->name->toString() === 'PHP_VERSION') {
            ...
            if ($minVersion !== null && $maxVersion !== null) {
                return [new ConstantStringType($minVersion->getVersionString()), new ConstantStringType($maxVersion->getVersionString())];
            }
        }

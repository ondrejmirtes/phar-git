...
final class ArrayChangeKeyCaseFunctionReturnTypeExtension implements DynamicFunctionReturnTypeExtension
{
    ...
    public function getTypeFromFunctionCall(FunctionReflection $functionReflection, FuncCall $functionCall, Scope $scope): ?Type
    {
        ...
        if (count($constantArrays) > 0) {
            ...
            foreach ($constantArrays as $constantArray) {
                ...
                foreach ($constantArray->getKeyTypes() as $i => $keyType) {
                    ...
                    if (count($constantStrings) > 0) {
                        $keyType = TypeCombinator::union(...array_map(fn(ConstantStringType $type): Type => $this->mapConstantString($type, $case), $constantStrings));
                    }
                    $newConstantArrayBuilder->setOffsetValueType($keyType, $valueType, $constantArray->isOptionalKey($i));
                }
            ...
            $keysType = TypeTraverser::map($keysType, function (Type $type, callable $traverse) use ($case): Type {
                ...
                if (count($constantStrings) > 0) {
                    return TypeCombinator::union(...array_map(fn(ConstantStringType $type): Type => $this->mapConstantString($type, $case), $constantStrings));
                }
            ...
            $newArrayType = TypeCombinator::intersect(new ArrayType($keysType, $arrayType->getIterableValueType()), ...TypeUtils::getAccessoryTypes($arrayType));
        }
    ...
    private function mapConstantString(ConstantStringType $type, ?int $case): Type
    {
        ...
        return TypeCombinator::union(new ConstantStringType(strtolower($type->getValue())), new ConstantStringType(strtoupper($type->getValue())));
    }
}

...
final class ArrayMapFunctionReturnTypeExtension implements DynamicFunctionReturnTypeExtension
{
    ...
    public function getTypeFromFunctionCall(FunctionReflection $functionReflection, FuncCall $functionCall, Scope $scope): ?Type
    {
        ...
        if ($callableType->isCallable()->yes()) {
            ...
            $valueType = ParametersAcceptorSelector::selectFromTypes(array_map(static fn(Node\Arg $arg) => $scope->getType($arg->value)->getIterableValueType(), array_slice($functionCall->getArgs(), 1)), $callableParametersAcceptors, \false)->getReturnType();
            ...
            if (!$areAllSameSize) {
                ...
                $and = array_reduce($identities, static fn(Node\Expr $a, Node\Expr $b) => new Node\Expr\BinaryOp\BooleanAnd($a, $b), new Node\Expr\ConstFetch(new Node\Name('true')));
            ...
            foreach ($argTypes as $index => $argType) {
                ...
                $arrayBuilder->setOffsetValueType(new ConstantIntegerType($index), $offsetValueType);
            }
        ...
        if ($singleArrayArgument) {
                ...
                if ($totalCount < ConstantArrayTypeBuilder::ARRAY_COUNT_LIMIT) {
                    foreach ($constantArrays as $constantArray) {
                        ...
                        foreach ($constantArray->getKeyTypes() as $i => $keyType) {
                            $returnedArrayBuilder->setOffsetValueType($keyType, $callableParametersAcceptors !== null ? ParametersAcceptorSelector::selectFromTypes([$valueTypes[$i]], $callableParametersAcceptors, \false)->getReturnType() : $valueType, $constantArray->isOptionalKey($i));
                        }
            ...
            $mappedArrayType = TypeCombinator::intersect(new ArrayType(new IntegerType(), $valueType), new AccessoryArrayListType(), ...TypeUtils::getAccessoryTypes($arrayType));
        }

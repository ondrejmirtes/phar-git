...
final class MbStrlenFunctionReturnTypeExtension implements DynamicFunctionReturnTypeExtension
{
    ...
    public function getTypeFromFunctionCall(FunctionReflection $functionReflection, FuncCall $functionCall, Scope $scope) : ?Type
    {
        ...
        if (count($functionCall->getArgs()) === 1) {
            ...
            $encodings = array_map(static fn(ConstantStringType $t) => $t->getValue(), $scope->getType($functionCall->getArgs()[1]->value)->getConstantStrings());
        }
        ...
        foreach ($constantScalars as $constantScalar) {
            ...
            foreach ($encodings as $encoding) {
                ...
                if ($length === \false) {
                    throw new ShouldNotHappenException(sprintf('Got false on a supported encoding %s and value %s', $encoding, var_export($stringScalar, \true)));
                }
                $lengths[] = $length;
            }
        }
        ...
        if (count($lengths) > 0) {
            ...
            $range = TypeCombinator::remove(ParametersAcceptorSelector::selectFromArgs($scope, $functionCall->getArgs(), $functionReflection->getVariants())->getReturnType(), new ConstantBooleanType(\false));
        }

...
final class SprintfFunctionDynamicReturnTypeExtension implements DynamicFunctionReturnTypeExtension
{
    ...
    public function getTypeFromFunctionCall(FunctionReflection $functionReflection, FuncCall $functionCall, Scope $scope) : ?Type
    {
        ...
        $isLowercase = $formatType->isLowercaseString()->yes() && $this->allValuesSatisfies($functionReflection, $scope, $args, static fn(Type $type): bool => $type->toString()->isLowercaseString()->yes());
        ...
        foreach ($formatStrings as $constantString) {
            $constantParts = $this->getFormatConstantParts($constantString->getValue(), $functionReflection, $functionCall, $scope);
            ...
            if (is_array($singlePlaceholderEarlyReturn) && preg_match('/^%(?P<argnum>[0-9]*\\$)?(?P<width>[0-9]*)\\.?[0-9]*(?P<specifier>[sbdeEfFgGhHouxX])$/', $constantString->getValue(), $matches) === 1) {
                ...
                if ($matches['specifier'] === 's' && ($checkArgType->isString()->yes() || $checkArgType->isInteger()->yes())) {
                    ...
                    $singlePlaceholderEarlyReturn[] = $this->getStringReturnType(new AccessoryNumericStringType(), $isLowercase);
                }
                continue;
            }
        ...
        if (!$isNonEmpty && $formatType->isNonEmptyString()->yes()) {
            $isNonEmpty = $this->allValuesSatisfies($functionReflection, $scope, $args, static fn(Type $type): bool => $type->toString()->isNonEmptyString()->yes());
        }
        if ($isNonEmpty) {
    ...
    private function getFormatConstantParts(string $format, FunctionReflection $functionReflection, FuncCall $functionCall, Scope $scope) : ?ConstantStringType
    {
        ...
        } elseif ($functionReflection->getName() === 'vsprintf' && count($args) >= 2) {
        ...
        try {
            ...
            if ($formatted === \false) {
                // @phpstan-ignore identical.alwaysFalse (PHP7.2 compat)
                return null;
            }

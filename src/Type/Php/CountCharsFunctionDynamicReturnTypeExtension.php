...
final class CountCharsFunctionDynamicReturnTypeExtension implements DynamicFunctionReturnTypeExtension
{
    ...
    public function getTypeFromFunctionCall(FunctionReflection $functionReflection, FuncCall $functionCall, Scope $scope): ?Type
    {
        ...
        if (IntegerRangeType::fromInterval(0, 2)->isSuperTypeOf($modeType)->yes()) {
            ...
            return $this->phpVersion->throwsValueErrorForInternalFunctions() ? $arrayType : TypeUtils::toBenevolentUnion(new UnionType([$arrayType, new ConstantBooleanType(\false)]));
        }
        $stringType = new StringType();
        return $this->phpVersion->throwsValueErrorForInternalFunctions() ? $stringType : TypeUtils::toBenevolentUnion(new UnionType([$stringType, new ConstantBooleanType(\false)]));
    }
}

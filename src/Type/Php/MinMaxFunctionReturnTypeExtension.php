...
final class MinMaxFunctionReturnTypeExtension implements DynamicFunctionReturnTypeExtension
{
    ...
    public function getTypeFromFunctionCall(FunctionReflection $functionReflection, FuncCall $functionCall, Scope $scope): ?Type
    {
        ...
        if (count($functionCall->getArgs()) === 2) {
            ...
            if ($argType0->isArray()->no() && $argType1->isArray()->no()) {
                $comparisonExpr = new Smaller(new AlwaysRememberedExpr($args[0]->value, $argType0, $scope->getNativeType($args[0]->value)), new AlwaysRememberedExpr($args[1]->value, $argType1, $scope->getNativeType($args[1]->value)));
                if ($functionName === 'min') {
                    return $scope->getType(new Ternary($comparisonExpr, $args[0]->value, $args[1]->value));
                } elseif ($functionName === 'max') {
                    return $scope->getType(new Ternary($comparisonExpr, $args[1]->value, $args[0]->value));
                }
            }
        }
        $argumentTypes = [];
    ...
    private function compareTypes(Type $firstType, Type $secondType): ?Type
    {
        ...
        if ($firstType->isConstantScalarValue()->yes() && $secondType->isArray()->yes()) {
            return $firstType;
        }

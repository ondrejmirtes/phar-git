...
final class ReplaceFunctionsDynamicReturnTypeExtension implements DynamicFunctionReturnTypeExtension
{
        ...
        if ($compareSuperTypes === $isStringSuperType) {
            ...
            if (count($subjectArrays) > 0) {
                ...
                foreach ($subjectArrays as $arrayType) {
                    ...
                    if ($constantArrays !== [] && in_array($functionReflection->getName(), ['preg_replace', 'preg_replace_callback', 'preg_replace_callback_array'], \true)) {
                        foreach ($constantArrays as $constantArray) {
                            $generalizedArray = $constantArray->generalizeValues();
                            $builder = ConstantArrayTypeBuilder::createEmpty();
                            // turn all keys optional
                            foreach ($constantArray->getKeyTypes() as $keyType) {
                                $builder->setOffsetValueType($keyType, $generalizedArray->getOffsetValueType($keyType), \true);
                            }
    ...
    private function canReturnNull(FunctionReflection $functionReflection, FuncCall $functionCall, Scope $scope): bool
    {
        if (in_array($functionReflection->getName(), ['preg_replace', 'preg_replace_callback', 'preg_replace_callback_array'], \true) && count($functionCall->getArgs()) > 0) {
            $subjectArgumentType = $this->getSubjectType($functionReflection, $functionCall, $scope);
            if ($subjectArgumentType !== null && $subjectArgumentType->isArray()->yes()) {
                return \false;
            }
        }

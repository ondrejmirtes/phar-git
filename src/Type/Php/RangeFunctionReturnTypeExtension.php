...
final class RangeFunctionReturnTypeExtension implements DynamicFunctionReturnTypeExtension
{
    ...
    public function getTypeFromFunctionCall(FunctionReflection $functionReflection, FuncCall $functionCall, Scope $scope): ?Type
    {
            ...
            foreach ($endConstants as $endConstant) {
                ...
                foreach ($stepConstants as $stepConstant) {
                    ...
                    if (count($rangeValues) > self::RANGE_LENGTH_THRESHOLD) {
                        if ($startConstant instanceof ConstantIntegerType && $endConstant instanceof ConstantIntegerType && $stepConstant instanceof ConstantIntegerType) {
                            if ($startConstant->getValue() > $endConstant->getValue()) {
                                $tmp = $startConstant;
                                $startConstant = $endConstant;
                                $endConstant = $tmp;
                            }
                            return TypeCombinator::intersect(new ArrayType(new IntegerType(), IntegerRangeType::fromInterval($startConstant->getValue(), $endConstant->getValue())), new NonEmptyArrayType(), new AccessoryArrayListType());
                        }
                        if ($stepType->isFloat()->yes()) {
                            return TypeCombinator::intersect(new ArrayType(new IntegerType(), new FloatType()), new NonEmptyArrayType(), new AccessoryArrayListType());
                        }
                        return TypeCombinator::intersect(new ArrayType(new IntegerType(), TypeCombinator::union($startConstant->generalize(GeneralizePrecision::moreSpecific()), $endConstant->generalize(GeneralizePrecision::moreSpecific()), $stepType->generalize(GeneralizePrecision::moreSpecific()))), new NonEmptyArrayType(), new AccessoryArrayListType());
                    }
        ...
        return TypeCombinator::intersect(new ArrayType(new IntegerType(), new BenevolentUnionType([new IntegerType(), new FloatType(), new StringType()])), new AccessoryArrayListType());
    }
}

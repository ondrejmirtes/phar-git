    ...
    public function specifyTypes(FunctionReflection $functionReflection, FuncCall $node, Scope $scope, TypeSpecifierContext $context) : SpecifiedTypes
    {
        ...
        if (count($args) >= 2) {
            ...
            if ($needleType->isNonEmptyString()->yes() && $haystackType->isString()->yes()) {
                ...
                return $this->typeSpecifier->create($args[$hackstackArg]->value, new IntersectionType($accessories), $context, $scope)->setRootExpr(new BooleanAnd(new NotIdentical($args[$needleArg]->value, new String_('')), new FuncCall(new Name('FAUX_FUNCTION'), [new Arg($args[$needleArg]->value)])));
            }
        }

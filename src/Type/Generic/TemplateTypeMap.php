...
final class TemplateTypeMap
{
    ...
    private static ?\PHPStan\Type\Generic\TemplateTypeMap $empty = null;
    private ?\PHPStan\Type\Generic\TemplateTypeMap $resolvedToBounds = null;
    /**
     * @api
    ...
    public function resolveToBounds() : self
    {
        ...
        return $this->resolvedToBounds = $this->map(static fn(string $name, Type $type): Type => TypeTraverser::map($type, static fn(Type $type, callable $traverse): Type => $type instanceof \PHPStan\Type\Generic\TemplateType ? $traverse($type->getDefault() ?? $type->getBound()) : $traverse($type)));
    }
}

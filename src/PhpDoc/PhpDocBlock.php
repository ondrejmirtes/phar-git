...
final class PhpDocBlock
{
     ...
     */
    private function __construct(string $docComment, ?string $file, ClassReflection $classReflection, ?string $trait, array $parameterNameMapping, array $parents)
    {
    ...
    public static function resolvePhpDocBlockForProperty(?string $docComment, ClassReflection $classReflection, ?string $trait, string $propertyName, ?string $file) : self
    {
        ...
        foreach (self::getParentReflections($classReflection) as $parentReflection) {
            $oneResult = self::resolvePropertyPhpDocBlockFromClass($parentReflection, $propertyName);
            if ($oneResult === null) {
    ...
    public static function resolvePhpDocBlockForConstant(?string $docComment, ClassReflection $classReflection, string $constantName, ?string $file) : self
    {
        $docBlocksFromParents = [];
        foreach (self::getParentReflections($classReflection) as $parentReflection) {
            $oneResult = self::resolveConstantPhpDocBlockFromClass($parentReflection, $constantName);
            if ($oneResult === null) {
                // Null if it is private or from a wrong trait.
                continue;
            }
        ...
        return new self($docComment ?? \PHPStan\PhpDoc\ResolvedPhpDocBlock::EMPTY_DOC_STRING, $file, $classReflection, null, [], $docBlocksFromParents);
    }
    /**
    ...
    public static function resolvePhpDocBlockForMethod(?string $docComment, ClassReflection $classReflection, ?string $trait, string $methodName, ?string $file, array $originalPositionalParameterNames, array $newPositionalParameterNames) : self
    {
        $docBlocksFromParents = [];
        foreach (self::getParentReflections($classReflection) as $parentReflection) {
            $oneResult = self::resolveMethodPhpDocBlockFromClass($parentReflection, $methodName, $newPositionalParameterNames);
            if ($oneResult === null) {
                // Null if it is private or from a wrong trait.
                continue;
            }
        ...
        foreach ($classReflection->getTraits(\true) as $traitReflection) {
            ...
            $docBlocksFromParents[] = new self($traitMethod->getDocComment() ?? \PHPStan\PhpDoc\ResolvedPhpDocBlock::EMPTY_DOC_STRING, $classReflection->getFileName(), $classReflection, $traitReflection->getName(), self::remapParameterNames($newPositionalParameterNames, $positionalMethodParameterNames), []);
        }
        return new self($docComment ?? \PHPStan\PhpDoc\ResolvedPhpDocBlock::EMPTY_DOC_STRING, $file, $classReflection, $trait, self::remapParameterNames($originalPositionalParameterNames, $newPositionalParameterNames), $docBlocksFromParents);
    }
    /**
     * @param array<int, string> $originalPositionalParameterNames
     * @param array<int, string> $newPositionalParameterNames
    ...
    private static function resolveConstantPhpDocBlockFromClass(ClassReflection $classReflection, string $name) : ?self
    {
        if ($classReflection->hasConstant($name)) {
            ...
            return self::resolvePhpDocBlockForConstant($parentReflection->getDocComment() ?? \PHPStan\PhpDoc\ResolvedPhpDocBlock::EMPTY_DOC_STRING, $classReflection, $name, $classReflection->getFileName());
        }
        return null;
    }
    private static function resolvePropertyPhpDocBlockFromClass(ClassReflection $classReflection, string $name) : ?self
    {
        if ($classReflection->hasNativeProperty($name)) {
            ...
            return self::resolvePhpDocBlockForProperty($parentReflection->getDocComment() ?? \PHPStan\PhpDoc\ResolvedPhpDocBlock::EMPTY_DOC_STRING, $classReflection, $trait, $name, $classReflection->getFileName());
        }
        return null;
    }
    /**
    ...
    private static function resolveMethodPhpDocBlockFromClass(ClassReflection $classReflection, string $name, array $positionalParameterNames) : ?self
    {
        if ($classReflection->hasNativeMethod($name)) {
            ...
            $trait = $traitReflection !== null ? $traitReflection->getName() : null;
            return self::resolvePhpDocBlockForMethod($parentReflection->getDocComment() ?? \PHPStan\PhpDoc\ResolvedPhpDocBlock::EMPTY_DOC_STRING, $classReflection, $trait, $name, $classReflection->getFileName(), $positionalParameterNames, $positionalMethodParameterNames);
        }
        return null;
    }
}

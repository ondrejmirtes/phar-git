    ...
    private function resolveIdentifierTypeNode(IdentifierTypeNode $typeNode, NameScope $nameScope) : Type
    {
        switch (strtolower($typeNode->name)) {
            ...
            case 'non-empty-lowercase-string':
                return new IntersectionType([new StringType(), new AccessoryNonEmptyStringType(), new AccessoryLowercaseStringType()]);
            case 'non-empty-uppercase-string':
                return new IntersectionType([new StringType(), new AccessoryNonEmptyStringType(), new AccessoryUppercaseStringType()]);
            case 'truthy-string':
            ...
            case 'non-empty-list':
                return TypeCombinator::intersect(new ArrayType(IntegerRangeType::createAllGreaterThanOrEqualTo(0), new MixedType()), new NonEmptyArrayType(), new AccessoryArrayListType());
            case 'empty':
                $type = $this->tryResolvePseudoTypeClassType($typeNode, $nameScope);
                if ($type !== null) {
                    return $type;
        ...
        if (in_array($mainTypeName, ['array', 'non-empty-array'], \true)) {
            if (count($genericTypes) === 1) {
                ...
                if (count($finiteTypes) === 1 && ($finiteTypes[0] instanceof ConstantStringType || $finiteTypes[0] instanceof ConstantIntegerType)) {
                    ...
                    $arrayBuilder->setOffsetValueType($finiteTypes[0], $genericTypes[1], \true);
    ...
    private function resolveCallableTypeNode(CallableTypeNode $typeNode, NameScope $nameScope) : Type
    {
        ...
        if (count($typeNode->templateTypes) > 0) {
            foreach ($typeNode->templateTypes as $templateType) {
                $templateTags[$templateType->name] = new TemplateTag($templateType->name, $templateType->bound !== null ? $this->resolve($templateType->bound, $nameScope) : new MixedType(), $templateType->default !== null ? $this->resolve($templateType->default, $nameScope) : null, TemplateTypeVariance::createInvariant());
            }
        ...
        if ($mainType instanceof CallableType) {
            ...
            return new ClosureType($parameters, $returnType, $isVariadic, $templateTypeMap, null, null, $templateTags, [], [new SimpleImpurePoint('functionCall', 'call to a Closure', \false)]);

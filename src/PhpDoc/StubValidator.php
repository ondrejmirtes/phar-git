...
final class StubValidator
{
    ...
    public function validate(array $stubFiles, bool $debug): array
    {
        ...
        foreach ($stubFiles as $stubFile) {
            try {
                ...
                $errors[] = (new Error($internalErrorMessage, $stubFile, null, $e))->withIdentifier('phpstan.internal')->withMetadata([InternalError::STACK_TRACE_METADATA_KEY => InternalError::prepareTrace($e), InternalError::STACK_TRACE_AS_STRING_METADATA_KEY => $e->getTraceAsString()]);
            }
        }
    ...
    private function getRuleRegistry(Container $container): RuleRegistry
    {
        ...
        $methodTagCheck = new MethodTagCheck($reflectionProvider, $classNameCheck, $genericObjectTypeCheck, $missingTypehintCheck, $unresolvableTypeHelper, \true, \true, $discoveringSymbolsTip);
        $propertyTagCheck = new PropertyTagCheck($reflectionProvider, $classNameCheck, $genericObjectTypeCheck, $missingTypehintCheck, $unresolvableTypeHelper, \true, \true, $discoveringSymbolsTip);
        ...
        $rules = [
            ...
            new ExistingClassesInPropertiesRule($reflectionProvider, $classNameCheck, $unresolvableTypeHelper, $phpVersion, \true, \false, $discoveringSymbolsTip),
            new OverridingMethodRule($phpVersion, new MethodSignatureRule($phpClassReflectionExtension, \true, \true), \true, new MethodParameterComparisonHelper($phpVersion), new MethodVisibilityComparisonHelper(), $phpClassReflectionExtension, $container->getParameter('checkMissingOverrideMethodAttribute')),
            ...
            new InvalidPhpDocTagValueRule($container->getByType(Lexer::class), $container->getByType(PhpDocParser::class)),
            ...
            new InvalidPHPStanDocTagRule($container->getByType(Lexer::class), $container->getByType(PhpDocParser::class)),

...
final class StubPhpDocProvider
{
    ...
    private \PHPStan\PhpDoc\StubFilesProvider $stubFilesProvider;
     ...
     */
    public function findMethodPhpDoc(string $className, string $implementingClassName, string $methodName, array $positionalParameterNames) : ?\PHPStan\PhpDoc\ResolvedPhpDocBlock
    {
        if (!$this->isKnownClass($className)) {
            return null;
        }
        ...
        if (array_key_exists($methodName, $this->knownMethodsDocComments[$className])) {
            ...
            if ($className !== $implementingClassName && $resolvedPhpDoc->getNullableNameScope() !== null) {
                $resolvedPhpDoc = $resolvedPhpDoc->withNameScope($resolvedPhpDoc->getNullableNameScope()->withClassName($implementingClassName));
            }

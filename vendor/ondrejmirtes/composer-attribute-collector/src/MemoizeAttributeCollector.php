...
class MemoizeAttributeCollector
{
    ...
    public function collectAttributes(array $classMap): TransientCollection
    {
        ...
        foreach ($classMap as $class => $filepath) {
            $filterClasses[$class] = \true;
            [$timestamp, $classAttributes, $methodAttributes, $propertyAttributes] = $this->state[$class] ?? [0, [], [], []];
            ...
            if ($timestamp < $mtime) {
                ...
                try {
                    [$classAttributes, $methodAttributes, $propertyAttributes] = $classAttributeCollector->collectAttributes($class);
                } catch (Throwable $e) {
         ...
         */
        $this->state = array_filter($this->state, static fn(string $k): bool => $filterClasses[$k] ?? \false, ARRAY_FILTER_USE_KEY);

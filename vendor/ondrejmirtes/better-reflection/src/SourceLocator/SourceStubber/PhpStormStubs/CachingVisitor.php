...
class CachingVisitor extends NodeVisitorAbstract
{
    ...
    public function enterNode(Node $node) : ?int
    {
        ...
        if ($node instanceof Node\Stmt\Expression && $node->expr instanceof Node\Expr\FuncCall) {
            ...
            if (in_array($constantName, ['STDIN', 'STDOUT', 'STDERR'], \true) && array_key_exists(1, $functionCall->args) && $functionCall->args[1] instanceof Node\Arg) {
            ...
            if (array_key_exists(2, $functionCall->args) && $functionCall->args[2] instanceof Node\Arg && $functionCall->args[2]->value instanceof Node\Expr\ConstFetch && $functionCall->args[2]->value->name->toLowerString() === 'true') {

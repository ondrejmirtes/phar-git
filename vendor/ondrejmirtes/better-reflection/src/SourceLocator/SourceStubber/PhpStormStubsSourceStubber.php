...
use _PHPStan_checksum\JetBrains\PHPStormStub\PhpStormStubsMap;
...
final class PhpStormStubsSourceStubber implements \PHPStan\BetterReflection\SourceLocator\SourceStubber\SourceStubber
{
    ...
    private function parseFile(string $filePath): void
    {
        ...
        foreach ($this->cachingVisitor->getClassNodes() as $className => $classNodeData) {
            ...
            if ($isCoreExtension) {
                if ($className !== 'Attribute' && $className !== 'ReturnTypeWillChange' && $className !== 'AllowDynamicProperties' && $className !== 'SensitiveParameter' && $className !== 'Override' && $className !== 'Deprecated' && !$this->isSupportedInPhpVersion($classNode)) {
                    continue;
                }

...
class ReflectionClass implements \PHPStan\BetterReflection\Reflection\Reflection
{
    ...
    protected function __construct(Reflector $reflector, $node, LocatedSource $locatedSource, ?string $namespace = null)
    {
        ...
        $implementsClassNames = array_map(static fn(Node\Name $name): string => $name->toString(), $node instanceof TraitNode ? [] : ($node instanceof InterfaceNode ? $node->extends : $node->implements));
    ...
    private function createMethodsFromTrait(\PHPStan\BetterReflection\Reflection\ReflectionMethod $method, array $currentMethods): array
    {
        ...
        $methods = [];
        if (!array_key_exists($lowerCasedMethodHash, $this->traitsData['precedences']) && !array_key_exists($lowerCasedMethodHash, $currentMethods)) {
    ...
    private function getMethodsIndexedByLowercasedName(AlreadyVisitedClasses $alreadyVisitedClasses): array
    {
        ...
        $methods = array_combine(array_map(static fn(\PHPStan\BetterReflection\Reflection\ReflectionMethod $method): string => strtolower($method->getName()), $immediateMethods), $immediateMethods);
    ...
    public function getMethods(int $filter = 0): array
    {
        ...
        if ($filter !== 0) {
            $methods = array_filter($methods, static fn(\PHPStan\BetterReflection\Reflection\ReflectionMethod $method): bool => (bool) ($filter & $method->getModifiers()));
        }
        return array_combine(array_map(static fn(\PHPStan\BetterReflection\Reflection\ReflectionMethod $method): string => $method->getName(), $methods), $methods);
    }
    /**
    ...
    public function getImmediateMethods(int $filter = 0): array
    {
        ...
        return array_filter($this->immediateMethods, static fn(\PHPStan\BetterReflection\Reflection\ReflectionMethod $method): bool => (bool) ($filter & $method->getModifiers()));
    }
    /** @return array<non-empty-string, ReflectionMethod>
     * @param ClassNode|InterfaceNode|TraitNode|EnumNode $node */
    private function createImmediateMethods($node, Reflector $reflector): array
    {
        ...
        foreach ($node->getMethods() as $methodNode) {
            $method = \PHPStan\BetterReflection\Reflection\ReflectionMethod::createFromMethodNode($reflector, $methodNode, $this->locatedSource, $this->getNamespaceName(), $this, $this, $this);
    ...
    private function addEnumMethods(EnumNode $node, array $methods): array
    {
        ...
        $createMethod = function (string $name, array $params, $returnType) use ($internalLocatedSource): \PHPStan\BetterReflection\Reflection\ReflectionMethod {
            ...
            return \PHPStan\BetterReflection\Reflection\ReflectionMethod::createFromMethodNode($this->reflector, new ClassMethod(new Node\Identifier($name), $classMethodSubnodes), $internalLocatedSource, $this->getNamespaceName(), $this, $this, $this);
        };
    ...
    public function getImmediateConstants(int $filter = 0): array
    {
        ...
        return array_filter($this->immediateConstants, static fn(\PHPStan\BetterReflection\Reflection\ReflectionClassConstant $constant): bool => (bool) ($filter & $constant->getModifiers()));
    }
    /**
    ...
    public function getConstructor(): ?\PHPStan\BetterReflection\Reflection\ReflectionMethod
    {
        ...
        $constructors = array_values(array_filter($this->getMethods(), static fn(\PHPStan\BetterReflection\Reflection\ReflectionMethod $method): bool => $method->isConstructor()));
    ...
    public function getImmediateProperties(int $filter = 0): array
    {
        ...
        return array_filter($this->immediateProperties, static fn(\PHPStan\BetterReflection\Reflection\ReflectionProperty $property): bool => (bool) ($filter & $property->getModifiers()));
    }
    /** @return array<non-empty-string, ReflectionProperty>
     * @param ClassNode|InterfaceNode|TraitNode|EnumNode $node */
    private function createImmediateProperties($node, Reflector $reflector): array
    {
        ...
        foreach ($node->getMethods() as $methodNode) {
            ...
            foreach ($methodNode->params as $parameterNode) {
                ...
                $propertyNode = new Node\Stmt\Property($parameterNode->flags, [new Node\PropertyItem($parameterNameNode->name)], $parameterNode->getAttributes(), $parameterNode->type, $parameterNode->attrGroups, $parameterNode->hooks);
    ...
    private function getPropertiesConsideringAlreadyVisitedClasses(AlreadyVisitedClasses $alreadyVisitedClasses): array
    {
        ...
        $properties = array_merge(array_filter((($nullsafeVariable2 = $this->getParentClass()) ? $nullsafeVariable2->getPropertiesConsideringAlreadyVisitedClasses($alreadyVisitedClasses) : null) ?? [], static fn(\PHPStan\BetterReflection\Reflection\ReflectionProperty $property) => !$property->isPrivate()), ...array_map(static fn(\PHPStan\BetterReflection\Reflection\ReflectionClass $ancestor): array => $ancestor->getPropertiesConsideringAlreadyVisitedClasses(clone $alreadyVisitedClasses), array_values($this->getImmediateInterfaces())));
        foreach ($this->getTraits() as $trait) {
        ...
        foreach ($usedTraits as $usedTrait) {
            ...
            if ($adaptation instanceof Node\Stmt\TraitUseAdaptation\Alias) {
                ...
                if ($adaptation->newName !== null) {
                    ...
                    $traitsData['aliases'][$usedTraitClassName][] = ['alias' => $adaptation->newName->name, 'method' => $adaptation->method->toString(), 'hash' => $lowerCasedMethodHash];
                }
    ...
    public function getInterfaces(): array
    {
        ...
        $interfaces = array_merge([$this->getCurrentClassImplementedInterfacesIndexedByName()], array_map(static fn(self $parentClass): array => $parentClass->getCurrentClassImplementedInterfacesIndexedByName(), $this->getParentClasses()));
    ...
    public function getInterfaceNames(): array
    {
        ...
        return $this->cachedInterfaceNames = array_values(array_map(static fn(self $interface): string => $interface->getName(), $this->getInterfaces()));
    }
    /**

...
use _PHPStan_checksum\Nette\DI\Definitions\Definition;
use _PHPStan_checksum\Nette\DI\Definitions\Reference;
use _PHPStan_checksum\Nette\DI\Definitions\Statement;
use _PHPStan_checksum\Nette\PhpGenerator\Helpers as PhpHelpers;
use _PHPStan_checksum\Nette\Utils\Arrays;
use _PHPStan_checksum\Nette\Utils\Callback;
use _PHPStan_checksum\Nette\Utils\Reflection;
        ...
        foreach ($method->getParameters() as $num => $param) {
            ...
            if ($param->isVariadic()) {
                ...
                if ($param->isOptional()) {
                    ...
                    $res[$num] = null;
                    trigger_error(sprintf('The parameter %s should have a declared value in the configuration.', Reflection::toString($param)), \E_USER_DEPRECATED);
                }
            } else {

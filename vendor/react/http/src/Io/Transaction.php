...
namespace _PHPStan_checksum\React\Http\Io;

use _PHPStan_checksum\Psr\Http\Message\RequestInterface;
use _PHPStan_checksum\Psr\Http\Message\ResponseInterface;
use _PHPStan_checksum\Psr\Http\Message\UriInterface;
use _PHPStan_checksum\React\EventLoop\LoopInterface;
use _PHPStan_checksum\React\Http\Message\Response;
use _PHPStan_checksum\React\Http\Message\ResponseException;
use _PHPStan_checksum\React\Http\Message\Uri;
use _PHPStan_checksum\React\Promise\Deferred;
use _PHPStan_checksum\React\Promise\Promise;
...
class Transaction
{
    ...
    public function bufferResponse(ResponseInterface $response, Deferred $deferred, ClientRequestState $state)
    {
        ...
        if ($size !== null && $size > $this->maximumSize) {
            $body->close();
            return \_PHPStan_checksum\React\Promise\reject(new \OverflowException('Response body size of ' . $size . ' bytes exceeds maximum of ' . $this->maximumSize . ' bytes', \defined('SOCKET_EMSGSIZE') ? \SOCKET_EMSGSIZE : 90));
        }
        // body is not streaming => already buffered
        if (!$body instanceof ReadableStreamInterface) {
            return \_PHPStan_checksum\React\Promise\resolve($response);
        }
        /** @var ?\Closure $closer */
        $closer = null;

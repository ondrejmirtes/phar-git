...
namespace _PHPStan_checksum\React\Http\Message;

use _PHPStan_checksum\Fig\Http\Message\StatusCodeInterface;
use _PHPStan_checksum\Psr\Http\Message\ResponseInterface;
use _PHPStan_checksum\Psr\Http\Message\StreamInterface;
use _PHPStan_checksum\React\Http\Io\AbstractMessage;
use _PHPStan_checksum\React\Http\Io\BufferedBody;
use _PHPStan_checksum\React\Http\Io\HttpBodyStream;
...
final class Response extends AbstractMessage implements ResponseInterface, StatusCodeInterface
{
     ...
     */
    private static $phrasesMap = array(200 => 'OK', 203 => 'Non-Authoritative Information', 207 => 'Multi-Status', 226 => 'IM Used', 414 => 'URI Too Large', 418 => 'I\'m a teapot', 505 => 'HTTP Version Not Supported');
    /** @var int */
    private $statusCode;
    /** @var string */
    ...
    public static function parseMessage($message)
    {
        ...
        return new self((int) $start['status'], $headers, '', $start['version'], isset($start['reason']) ? $start['reason'] : '');
    }
}

...
namespace _PHPStan_checksum\React\Socket;

use _PHPStan_checksum\React\Dns\Config\Config as DnsConfig;
use _PHPStan_checksum\React\Dns\Resolver\Factory as DnsFactory;
use _PHPStan_checksum\React\Dns\Resolver\ResolverInterface;
...
final class Connector implements ConnectorInterface
{
    ...
    public function connect($uri)
    {
        ...
        if (!isset($this->connectors[$scheme])) {
            return \_PHPStan_checksum\React\Promise\reject(new \RuntimeException('No connector available for URI scheme "' . $scheme . '" (EINVAL)', \defined('SOCKET_EINVAL') ? \SOCKET_EINVAL : (\defined('PCNTL_EINVAL') ? \PCNTL_EINVAL : 22)));
        }

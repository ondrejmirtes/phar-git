...
use _PHPStan_checksum\React\Promise;
final class HappyEyeBallsConnector implements ConnectorInterface
{
    ...
    public function __construct($loop = null, $connector = null, $resolver = null)
    {
        ...
        if (!$connector instanceof ConnectorInterface) {
            // manual type check to support legacy PHP < 7.1
            throw new \InvalidArgumentException('_PHPStan_checksum\Argument #2 ($connector) expected React\Socket\ConnectorInterface');
        }
        if (!$resolver instanceof ResolverInterface) {
            // manual type check to support legacy PHP < 7.1
            throw new \InvalidArgumentException('_PHPStan_checksum\Argument #3 ($resolver) expected React\Dns\Resolver\ResolverInterface');
        }
